...
#
# S&W definitions (Sheffield & Wood 2008, eq. 2–4):
#   Intensity  I_e = (1/D_e) × Σ(threshold − x_t)  for t in event e
#   Severity   S_e = I_e × D_e  (= total accumulated deficit)
#   Reported pixel values are means across all events (any duration) at that pixel.
#
# Column names deliberately match the gap-analysis spec so w4 and w5 can refer
# to them directly without aliasing.
vectorized_duration_class_counts <- function(ts_matrix,
                                             threshold     = DROUGHT_ONSET,
                                             end_threshold = DROUGHT_END) {
  n_pix <- nrow(ts_matrix)
  res   <- data.table::data.table(
  ...
  for (i in seq_len(n_pix)) {
    ...
    # Entry  : x < threshold  (DROUGHT_ONSET, default −1.0)
    # Exit   : x >= end_threshold  (DROUGHT_END, default −0.5 or 0)
    # -------------------------------------------------------------------
    in_d   <- FALSE
    s_idx  <- NA_integer_
    durs   <- integer(0)
    ...
    for (j in seq_along(x)) {
      ...
      if (!in_d && !is.na(v) && v < threshold) {
        in_d  <- TRUE
        s_idx <- j
      } else if (in_d && !is.na(v) && v >= end_threshold) {
...
# ================================================================================
# NEW FUNCTION 7 — Basin-level % area in drought time series + MK trend
# ================================================================================
...
# @param ts_matrix   pixels × months matrix (already filtered to valid pixels)
# @param n_years     number of years in the record
# @param index_label e.g. "SPI-06"
# @param out_dir     directory for CSV/PNG outputs
# @return invisible list with extent_ts, mk_extent, freq_by_class, mk_by_class
compute_basin_extent_and_class_trends <- function(ts_matrix, n_years, index_label,
  ...
  # ================================================================================
  # FIX: Improved x-axis break calculation for all indices (SPI/SPEI/MSPI/MSPEI)
  # ================================================================================
  ...
  # Calculate 10-year breaks - ensure clean intervals
  first_break <- ceiling(x_min / 10) * 10
  last_break  <- floor(x_max / 10) * 10
  x_breaks    <- seq(first_break, last_break, by = 10)
  
  # Ensure breaks fall within data range
  x_breaks <- x_breaks[x_breaks >= x_min & x_breaks <= x_max]
  
  # Fallback: if no breaks generated, use min/max
  if (length(x_breaks) == 0) {
    x_breaks <- c(x_min, x_max)
  }
  
  # Limit maximum number of breaks to prevent overlap (max 15 labels)
  if (length(x_breaks) > 15) {
    step_size <- ceiling((x_max - x_min) / 10)
    step_size <- max(10, step_size)  # ensure at least 10-year intervals
  ...
  for (cl in dc_classes) {
    ...
    cat(sprintf(
      "  [Class MK | %s | %-25s] freq: tau=%.3f p=%.3f | sev: tau=%.3f p=%.3f\n",
      index_label, cl,
...
# --------------------------------------------------------------------------------
# NULL-coalesce helper (used inside compute_basin_extent_and_class_trends)
# --------------------------------------------------------------------------------
...
process_index <- function(index_type, scales, find_fn, seas_dir) {
  for (sc in scales) {
    ...
    # Columns flow directly into w4 for spatial frequency maps and into w5 for
    # the duration-class return period table and 2022-2025 spatial rank map.
    ...
    # Saves a single-layer raster so it can be visualised directly in QGIS
    # or in w4_trends_visualization.R.
...
find_swei_nc_safe <- function(data_dir, index_type, scale) {
  tryCatch(
    ...
    error = function(e) {
      ...
      character(0)   # return empty — process_index will skip gracefully
    }
  )
}
