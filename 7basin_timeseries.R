####################################################################################
# 7basin_timeseries.R  ·  BASIN-AVERAGED TIME SERIES FOR ALL INDICES
# ─────────────────────────────────────────────────────────────────────────────────
# SCOPE  (authoritative home for all basin-averaged outputs):
#   PART 1 – Load basin-averaged CSVs produced by 6trend_test.R for
...
# NOTE: 8trends_visualization.R handles ONLY spatial figures (Parts 1-2 of 8).
...
#   Step 2 — 6trend_test.R area-weights per-pixel values with
...
if (!length(all_data))
  stop("No basin-averaged data loaded. Run 6trend_test.R first.")
...
make_spei_panel_v2 <- function(sc, panel_lab, pet_type = "PM",
  ...
  # For Thornthwaite or any scale absent from 6, always use the seasonal CSVs
...
#       for scales absent from 6, from the per-month CSV files directly.
#
...
#
# SPI data path:
#   PRIMARY  : load_flat_csv("spi", sc)   (written by 6 for scales in SPI_SCALES)
...
cat("  Next: run 8trends_visualization.R for spatial figures.\n\n")