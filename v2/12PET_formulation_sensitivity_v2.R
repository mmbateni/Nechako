################################################################################
# 12PET_formulation_sensitivity_v2.R
...
#   Penman-Monteith PET (SPEI_PM) and Thornthwaite PET (SPEI_THW).
#
# IMPORTANT INTERPRETATION
#   delta_spei_pet = SPEI_PM - SPEI_THW is a formulation-sensitivity residual.
...
#     spei_results_seasonal_thw/spei_thw_XX_basin_averaged_by_month.csv
#
...
#   Fig_PET_formulation_sensitivity_timeseries.pdf/.png
#   Fig_PET_formulation_agreement_metrics.pdf/.png
################################################################################
...
load_pair <- function(scale) {
  ...
  thw <- load_basin_avg_csv("spei_thw", scale)
  if (is.null(pm) || is.null(thw))
  ...
  validate_monthly_record(thw$date, label = paste0("SPEI_THW-", scale))
  
  z <- merge(pm, thw, by = "date", all = TRUE, suffixes = c("_pm", "_thw"))
  ...
  z$SPEI_THW <- as.numeric(z$value_thw)
  z$delta_spei_pet <- z$SPEI_PM - z$SPEI_THW
  z$abs_delta_spei_pet <- abs(z$delta_spei_pet)
  z$drought_pm  <- is.finite(z$SPEI_PM)  & z$SPEI_PM  < DROUGHT_THR
  z$drought_thw <- is.finite(z$SPEI_THW) & z$SPEI_THW < DROUGHT_THR
  z$drought_active <- z$drought_pm | z$drought_thw
  z
}
...
metric_row <- function(df, scale, period) {
  ok <- is.finite(df$SPEI_PM) & is.finite(df$SPEI_THW)
  x <- df$SPEI_PM[ok]
  y <- df$SPEI_THW[ok]
  n <- length(x)
...
bootstrap_metric_ci <- function(
  ...
  ok <- is.finite(df$SPEI_PM) &
    is.finite(df$SPEI_THW)
  
  d <- df[ok, , drop = FALSE]
  
  x <- as.numeric(d$SPEI_PM)
  y <- as.numeric(d$SPEI_THW)
...
# resulting statistic remains a formulation-sensitivity diagnostic, not a causal
# drought decomposition.
# ------------------------------------------------------------------------------
intensity_tbl <- all_ts |>
  dplyr::mutate(
    mean_spei = (SPEI_PM + SPEI_THW) / 2,
...
pdat <- all_ts |>
  ...
  tidyr::pivot_longer(c(SPEI_PM, SPEI_THW), names_to = "formulation",
                      values_to = "SPEI") |>
  dplyr::mutate(
    formulation = factor(formulation, c("SPEI_PM", "SPEI_THW"),
                         c("Penman-Monteith", "Thornthwaite")),
...
p1 <- ggplot2::ggplot(pdat, ggplot2::aes(date, SPEI, linetype = formulation)) +
  ...
  ggplot2::labs(x = NULL, y = "Basin-mean SPEI", linetype = "PET formulation",
                title = "SPEI sensitivity to PET formulation",
                subtitle = "PM and Thornthwaite are independently standardized") +
  ggplot2::theme_bw(base_size = 10) +
  ggplot2::theme(legend.position = "bottom")

ggplot2::ggsave(file.path(OUT_DIR, "Fig_PET_formulation_sensitivity_timeseries.png"),
                p1, width = 11, height = 9, dpi = FIG_DPI)
ggplot2::ggsave(file.path(OUT_DIR, "Fig_PET_formulation_sensitivity_timeseries.pdf"),
...
p2 <- ggplot2::ggplot(mplot, ggplot2::aes(factor(scale), value, group = period,
  ...
  ggplot2::labs(x = "Accumulation scale (months)", y = NULL,
                linetype = "Period",
                title = "PET-formulation agreement metrics") +
  ggplot2::theme_bw(base_size = 10) +
  ggplot2::theme(legend.position = "bottom")

ggplot2::ggsave(file.path(OUT_DIR, "Fig_PET_formulation_agreement_metrics.png"),
                p2, width = 10, height = 7, dpi = FIG_DPI)
ggplot2::ggsave(file.path(OUT_DIR, "Fig_PET_formulation_agreement_metrics.pdf"),
                p2, width = 10, height = 7)

cat("\n12PET_formulation_sensitivity_v2.R completed.\n")
cat("Outputs:", OUT_DIR, "\n")
