...
# ── Thornthwaite PET floor-safeguard configuration ───────────────────────────
# Define the PET path BEFORE any existence check or raster loading.
THW_FLOOR_PRIMARY       <- TRUE
THW_FLOOR_EPS_MM_MONTH  <- 1e-8
THW_FLOOR_MIN_OBS       <- 36L

THW_CANONICAL_MASK_FILE <- file.path(
  WD_PATH,
  "trend_analysis_pr_pet",
  "canonical_basin_mask.rds"
)

THW_PET_FILE <- file.path(
  WD_PATH,
  "monthly_data_direct",
  "potential_evapotranspiration_thornthwaite_monthly.nc"
)

if (!file.exists(THW_PET_FILE)) {
  stop(
    "Thornthwaite PET file required for the SPEI-Thw-01 floor safeguard ",
    "was not found:\n  ",
    THW_PET_FILE,
    "\nRun 2b_PET_ERALand_v2.R first."
  )
}

thw_pet_full_for_trend <- NULL
...
# SPEI_Thw NetCDF files are written by 3SPEI_ERALand_v2.R into
# spei_results_seasonal_thw/ alongside the PM SPEI seasonal files.  The index_type
# "spei_thw" is used consistently in downstream scripts (7, 8, 10a).
SPEI_THW_SEAS_DIR <- file.path(WD_PATH, "spei_results_seasonal_thw")
SPEI_THW_SCALES   <- c(1L, 3L, 6L, 12L, 24L, 36L)

# Thornthwaite zero-PET floor safeguard for SPEI-Thw-01 trend inference.
# The physical Thornthwaite formulation is NOT altered. For the manuscript-facing
# trend test at scale 1, months in which PET_Thw is at its 0-C floor are excluded
# from the sensitivity trend. The original full-record MK statistics are retained
# as *_raw columns for auditability. This prevents the winter freeze-floor
# transition from being presented as an unqualified PET-formulation trend.
# --------------------------------------------------------------------------------
#   Utility operators
# --------------------------------------------------------------------------------
...
process_index <- function(index_type, scales, find_fn, seas_dir) {
  for (sc in scales) {
    ...
    if (!file.exists(THW_CANONICAL_MASK_FILE)) {
      stop(
        "Canonical basin mask not found: ",
        THW_CANONICAL_MASK_FILE,
    ...
    canonical_mask <- readRDS(THW_CANONICAL_MASK_FILE)
    ...
    # SPEI_Thw block here instead reports "n_pix" valid (non-NA) pixels out of
    ...
    # -------------------------------------------------------------------------
    # Thornthwaite floor-aware trend sensitivity for SPEI-Thw-01.
    # Only the trend inference is screened; the SPEI field itself is unchanged.
    # -------------------------------------------------------------------------
    if (identical(tolower(index_type), "spei_thw") && identical(as.integer(sc), 1L)) {
      cat("  → Thornthwaite 0-C-floor-aware SPEI-01 trend sensitivity...    ")
      thw_pet <- thw_pet_full_for_trend
      pet_dates <- tryCatch(as.Date(terra::time(thw_pet)), error = function(e) NULL)
      if (is.null(pet_dates) || length(pet_dates) != terra::nlyr(thw_pet) || all(is.na(pet_dates))) {
        pet_dates <- seq(min(dates_ts), by = "month", length.out = terra::nlyr(thw_pet))
      } else {
        pet_dates <- as.Date(format(pet_dates, "%Y-%m-01"))
      }
      dates_key <- as.Date(format(dates_ts, "%Y-%m-01"))
      pet_idx <- match(dates_key, pet_dates)
      if (anyNA(pet_idx))
        stop("Could not align Thornthwaite PET dates with SPEI-Thw-01 trend dates.")
      
      # Nearest-neighbour transfer preserves the physical zero/non-zero floor state.
      thw_pet_grid <- terra::project(thw_pet, r_tmpl, method = "near")
      thw_pet_vals <- terra::values(thw_pet_grid, mat = TRUE)
      floor_full   <- thw_pet_vals[, pet_idx, drop = FALSE] <= THW_FLOOR_EPS_MM_MONTH
      floor_mat    <- floor_full[valid_pix, , drop = FALSE]
      active_ts    <- ts_mat
      active_ts[floor_mat] <- NA_real_
      active_n     <- rowSums(is.finite(active_ts))
      
      active_vc <- vectorized_mann_kendall(active_ts)
      active_tf <- vectorized_mann_kendall_tfpw(active_ts)
      
      # Require enough PET-active observations for a stable 76-year trend estimate.
      too_few <- active_n < THW_FLOOR_MIN_OBS
      if (any(too_few)) {
        active_vc[too_few, `:=`(tau_vc = NA_real_, p_value_vc = NA_real_,
                                sl_vc = NA_real_, filtered_vc = TRUE)]
        active_tf[too_few, `:=`(tau_tfpw = NA_real_, p_value_tfpw = NA_real_,
                                sl_tfpw = NA_real_, filtered_tfpw = TRUE)]
      }
      
      results[, `:=`(thw_pet_active_n = active_n,
                     thw_pet_floor_n = rowSums(floor_mat),
                     thw_pet_floor_fraction = rowMeans(floor_mat),
                     tau_vc_floor_adj = active_vc$tau_vc,
                     p_value_vc_floor_adj = active_vc$p_value_vc,
                     sl_vc_floor_adj = active_vc$sl_vc,
                     filtered_vc_floor_adj = active_vc$filtered_vc,
                     tau_tfpw_floor_adj = active_tf$tau_tfpw,
                     p_value_tfpw_floor_adj = active_tf$p_value_tfpw,
                     sl_tfpw_floor_adj = active_tf$sl_tfpw,
                     filtered_tfpw_floor_adj = active_tf$filtered_tfpw)]
      
      if (isTRUE(THW_FLOOR_PRIMARY)) {
        # Keep the ordinary full-record tau/Sen-slope fields unchanged because
        # the PET-active sensitivity series has irregular temporal spacing;
        # its p-values are the manuscript-facing significance gate. The
        # floor-adjusted tau/slope are retained in explicit *_floor_adj columns.
        results[, `:=`(p_value_vc = p_value_vc_floor_adj,
                       p_value_tfpw = p_value_tfpw_floor_adj)]
      }
      cat(sprintf("✓  primary trend = PET-active months; raw trend retained; median active n = %.0f\n",
                  median(active_n, na.rm = TRUE)))
    }
...
# SPEI_Thw — Thornthwaite PET variant written by 3SPEI_ERALand_v2.R
# into spei_results_seasonal_thw/.  Files may be named with index prefix
# "spei_thw" or, if 3SPEI_ERALand_v2.R wrote them with the plain "spei" prefix
