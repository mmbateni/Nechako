...
#             SPI, SPEI-PM, SPEI-Thw and optionally MSPI / MSPEI.
...
#             Fig2_MS_AllIndices_3x6.pdf/.png (18-panel: SPI, SPEI-PM, SPEI-Thw × 1, 3, 6, 12, 24, 36 months).
...
MSPI_MSPEI_SCALE <- 1L
if (!"spei_thw" %in% names(index_colours)) index_colours["spei_thw"] <- "#d62728"
...
cat("\n── SPEI-Thw (Thornthwaite; observed method sensitivity) ──\n")
for (sc in SPEI_SCALES) {
  ts <- load_basin_index("spei_thw", sc)
  if (!is.null(ts)) {
    ts$Index <- sprintf("SPEI_Thw%d", sc)
    all_data[[sprintf("SPEI_Thw%d", sc)]] <- ts
    cat(sprintf("  ✓ SPEI-Thw-%d: %d time steps\n", sc, nrow(ts)))
  }
}
...
make_basin_ts_plot <- function(df, idx, sc) {
  ...
  idx_label <- if (idx_lc == "spei") "SPEI-PM" else
    if (idx_lc == "spei_thw") "SPEI-Thw" else toupper(idx)
...
make_spei_panel_v2 <- function(sc, panel_lab, pet_type = "PM",
  ...
  idx_key <- if (pet_type == "PM") "spei" else "spei_thw"
...
tryCatch({
  ...
  cat("  Building Version B: SPEI-Thw (Thornthwaite)...\n")
  thw_panels <- list(
...
#       Row 3: SPEI-Thw-1, SPEI-Thw-3, SPEI-Thw-6, SPEI-Thw-12, SPEI-Thw-24, SPEI-Thw-36
...
#       Each panel overlays SPI, SPEI-PM, and SPEI-Thw for 2022–2025.
...
load_manuscript_ts_4e <- function(index_type, scale) {
  index_type <- match.arg(index_type, c("spi", "spei_pm", "spei_thw"))
  ...
  if (index_type == "spi") {
    ...
    df <- tryCatch(load_flat_csv("spei_thw", scale), error = function(e) NULL)
    if (is.null(df) || !nrow(df)) {
      df <- tryCatch(load_spei_seasonal_csvs(scale, SPEI_THW_DIR),
                     error = function(e) NULL)
    }
  }
...
make_historical_panel_4e <- function(sc, panel_lab, index_type,
  ...
  index_label <- switch(
    ...
    spei_thw = sprintf("SPEI-Thw-%d", sc)
  )
  
  line_col <- unname(LINE_COLOURS_4e[
    switch(index_type, spi = "SPI", spei_pm = "SPEI-PM", spei_thw = "SPEI-Thw")
  ])
...
save_historical_18panel_4e <- function(panels) {
  fig <- patchwork::wrap_plots(
    ...
    patchwork::plot_annotation(
      ...
      subtitle = paste0(
        "Rows: SPI, SPEI-PM, SPEI-Thw  |  Columns: 1-, 3-, 6-, 12-, 24-, 36-month accumulation scales  |  ",
...
tryCatch({
  ...
  index_grid <- rep(c("spi", "spei_pm", "spei_thw"), each = length(MANUSCRIPT_TS_SCALES))
...
make_recent_scale_panel_4e <- function(sc, panel_lab, show_xlab = FALSE,
  ...
  df_all$label <- factor(
    df_all$index_type,
    levels = c("spi", "spei_pm", "spei_thw"),
    labels = c("SPI", "SPEI-PM", "SPEI-Thw")
  )
...
tryCatch({
  ...
  fig_recent <- patchwork::wrap_plots(
    ...
    patchwork::plot_annotation(
      ...
      subtitle = paste0(
        "Each panel overlays SPI, SPEI-PM, and SPEI-Thw.  ",
...
REDUCED_HIST_INDEXES <- c("spi", "spei_pm", "spei_thw")
REDUCED_HIST_INDEX_LABELS <- c("SPI", "SPEI-PM", "SPEI-Thw")
...
if (!length(reduced_historical_data)) {
  stop(
    ...
    "Expected SPI, SPEI-PM, and SPEI-Thw at scales ",
...
#   * SPI/SPEI-PM/SPEI-Thw are implemented as the vertical y-axis titles of
...
build_9panel_historical_figure_4e <- function(scales, file_stub) {
  ...
  index_keys <- c("spi", "spei_pm", "spei_thw")
  index_labels <- c("SPI", "SPEI-PM", "SPEI-Thw")
...
S1_HIST_INDEXES <- c("spi", "spei_pm", "spei_thw")
S1_HIST_INDEX_LABELS <- c("SPI", "SPEI-PM", "SPEI-Thw")
...
if (!length(S1_HISTORICAL_DATA)) {
  stop(
    ...
    "and SPEI-Thw at scales 1 and 6 months."
  )
}
...
export_basin_excel <- function(output_file) {
  ...
  corr_df <- tryCatch({
    ...
    # SPEI (Thornthwaite)
    for (sc in SPEI_SCALES) {
      ts <- tryCatch(load_any_ts("spei_thw", sc), error = function(e) NULL)
      if (!is.null(ts))
        series_list[[sprintf("SPEI_Thw%d", sc)]] <- ts
    }
