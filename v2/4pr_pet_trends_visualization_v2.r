...
# ???????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????
# Reads results produced by 4pr_pet_trends.r (observed PM + Thornthwaite framework)
# and generates:
...
if (!has_pet_thw)
  cat("\n??????  PET_Thw NOT in all_results ??? re-run 4pr_pet_trends.r with Thw enabled.\n",
...
cat("???????  Generating rasters (Pr, PET_PM, PET_Thw, Temperature)...\n")
...
#   Precipitation, PET-PM, PET-Thornthwaite, and 2-m air temperature.
#
...
create_supp_hydroclimatic_trend_figure <- function(
  ...
  vars <- c("Precipitation", "PET", "PET_Thw", "Temperature")
  labels <- c(
    Precipitation = "Precipitation",
    PET           = "PET-PM",
    PET_Thw       = "PET-Thornthwaite",
  ...
  mm_d <- d[variable %in% c("Precipitation", "PET", "PET_Thw"), sl_tfpw]
...
create_comparison_figure <- function() {
  ...
  var_labels <- c(precip="Precipitation ??", pet="PET_PM ??",
                  pet_thw="PET_Thw ??", tair="Temperature ??")
...
create_pet_comparison_plots <- function() {
  if (!has_pet_thw) {
    cat("\n?????? PET_Thw not present ??? skipping Function 14.\n")
    return(invisible(NULL))
  }
  if (!"pet_thw_mm_month" %in% names(basin_avg_monthly)) {
    cat("\n?????? pet_thw_mm_month not in basin_avg_monthly ??? skipping Function 14.\n")
    return(invisible(NULL))
  }
