...
#   08KC001  Salmon River Near Prince George        ↔  GRDC 4207220
...
# (08JE001): Stuart River Near Fort St. James PASSED. The API successfully pulled data 
# all the way up to January 28, 2026. This confirms the gauge is actively transmitting and 
# your code works perfectly for fetching real-time/recent records.
# The Failures (The other 6 stations): Even through the live API, 
# stations like 08KC001 (Salmon River) cut off in late 2024, and 08JA015 (Laventie Creek) 
# cuts off at the end of 2023.
# ####################################################################################
...
# NOTE: dplyr verbs below (select/rename/filter/arrange/distinct/mutate) are
# explicitly namespaced as dplyr::___ . This script previously failed with
# "unused arguments" on select() because another loaded package in the R
# session (commonly `raster`, which defines its own select() for interactive
# extent-picking) was masking dplyr::select(). Namespacing avoids that.
...
fetch_wsc_paginated <- function(station_no, param, page_limit = 10000) {
  ...
  repeat {
    ...
    if (!"DATE" %in% names(page_rows) && "DATETIME" %in% names(page_rows))
      page_rows <- dplyr::rename(page_rows, DATE = DATETIME)
    ...
    page_rows <- page_rows %>%
      dplyr::select(Date = DATE, Value = all_of(param)) %>%
      dplyr::mutate(Date = as.Date(str_sub(as.character(Date), 1, 10))) %>%
  ...
  bind_rows(all_rows) %>% dplyr::arrange(Date) %>% dplyr::distinct(Date, .keep_all = TRUE)
}
...
download_station <- function(STATION_NUMBER, STATION_NAME,
  ...
  for (param in c("DISCHARGE", "LEVEL")) {
    ...
    if (!is.null(df) && nrow(df) > 0) {
      ...
      df       <- dplyr::rename(df, !!col_name := Value)
...
validation <- stations_df %>%
  ...
  mutate(
    ...
    # NOTE: GRDC's station catalogue d_start/d_end are YEAR-level fields
    # ("daily data available from year" / "until year") — GRDC does not
    # publish exact start/end dates, so we hard-coded Jan-1/Dec-31 as
    # stand-ins. Comparing those synthetic dates against WSC's *exact*
    # start/end dates penalizes stations whose gauge simply began partway
    # through its first calendar year (the normal case). Coverage is
    # therefore checked at calendar-year granularity, matching GRDC's
    # actual precision.
...
report <- validation %>%
  ...
  dplyr::mutate(across(c(grdc_d_start, grdc_d_end, wsc_start, wsc_end), as.character))
