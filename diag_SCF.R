...
# Compute diagnostics for a single candidate threshold and timescale
compute_diagnostics <- function(scf_clim_list, swe_kmat, dates, threshold, 
                                stability_years = 0.70) {
  months <- as.integer(format(dates, "%m"))
  ncell <- nrow(swe_kmat)
  nt <- ncol(swe_kmat)
  centers <- 1:12
  
  retained_frac <- numeric(12)
  ...
  for (m in centers) {
    ...
    if (is.null(r)) {
      ...
      vals <- values(r)
      mask_month[[m]] <- !is.na(vals) & (vals >= threshold)
      retained_frac[m] <- mean(mask_month[[m]], na.rm = TRUE)
    }
  }
...
score_candidate <- function(diag, targets = list(nonzero_years = 20, 
  ...
  # Retained area preference: moderate (not too small)
  mean_retained <- mean(diag$retained_frac, na.rm = TRUE)
...
candidates <- list(
  "1"  = c(0.03, 0.05, 0.075),
  "3"  = c(0.05, 0.075, 0.10),
  "6"  = c(0.075, 0.10, 0.125),
  "12" = c(0.10, 0.15, 0.20)
)

# Targets for scoring (adjustable)
targets <- list(nonzero_years = 20, zero_frac = 0.30, stability_pct = 70)
...
for (k in timescales) {
  ...
  cat(sprintf("\n??? Best threshold for %d-month: %.3f (score %.3f)\n", 
              k, best_th, max(scores)))
}
...
for (k in names(results_all)) {
  ...
  for (th_name in names(res_k$candidates)) {
    ...
    summary_rows[[length(summary_rows) + 1]] <- data.frame(
      ...
      mean_retained = mean(diag$retained_frac, na.rm = TRUE),
      min_retained = min(diag$retained_frac, na.rm = TRUE),
      max_retained = max(diag$retained_frac, na.rm = TRUE),
...
# ==============================================================================
# PLOT: RETAINED FRACTION BY TIMESCALE
# ==============================================================================

p3 <- ggplot(summary_df, aes(x = factor(threshold), y = mean_retained)) +
  geom_col(aes(fill = factor(timescale)), position = position_dodge()) +
  facet_wrap(~ timescale, scales = "free_x") +
  scale_y_continuous(labels = scales::percent) +
  labs(
    title = "Mean Retained Pixel Fraction",
    subtitle = "Average across 12 center months",
    x = "SCF Threshold",
    y = "Retained Fraction"
  ) +
  theme_minimal(base_size = 12) +
...
for (i in 1:nrow(final_table)) {
  ...
  cat(sprintf("  Mean retained: %.1f%%\n", 100 * row$mean_retained))
...
cat("INTERPRETATION:\n")
cat("===============\n")
cat("??? Higher retained fractions indicate more pixels included in analysis\n")
cat("??? Lower zero fractions indicate better snow persistence\n")
cat("??? Higher nonzero years indicate reliable long-term snow presence\n")
cat("??? Stability % shows fraction of pixels meeting consistency threshold\n\n")

cat("FILES GENERATED:\n")
cat("================\n")
cat("  1. scf_timescale_threshold_summary.csv - All candidates\n")
cat("  2. scf_timescale_final_recommendations.csv - Best choices\n")
cat("  3. scf_threshold_scores.png - Score comparison\n")
cat("  4. scf_diagnostic_nonzero_years.png - Persistence diagnostic\n")
cat("  5. scf_retained_fraction.png - Spatial coverage\n")
...
cat("  ??? scf_timescale_final_recommendations.csv - Recommended thresholds\n")
cat("  ??? scf_threshold_scores.png - Visual comparison\n")
cat("  ??? scf_decision_summary.txt - Full report\n")
cat("\n????????? USE THESE THRESHOLDS FOR SWEI CALCULATION ?????????\n")