#============================================================================
# QUICK-START & DATA VERIFICATION (SPI/SPEI/SSPI/SSMI)
#============================================================================
library(terra); library(copula)
setwd("D:/Nechako_Drought/Nechako")
cat(paste(rep("=",70), collapse=""), "\nSPI/SPEI/SSPI/SSMI VERIFICATION\n",
    paste(rep("=",70), collapse=""), "\n")

# SSMI layer config to check (must match SSMI_LAYER in A03_saf_core_pipeline.R)
SSMI_LAYER <- "L1_3"
...
files <- c(
  ...
  "sspi_Snow_results_monthly/sspi_01_month01_Jan.nc",
  sprintf("ssmi_results_seasonal/ssmi_%s_01_month01_Jan.nc", SSMI_LAYER)
)
missing <- files[!file.exists(files)]
if (length(missing) > 0) {
  cat("✗ Missing required NetCDFs:\n")
  cat(paste0("    - ", missing, collapse = "\n"), "\n")
  if (any(grepl("^sspi_results_monthly", missing)))
    cat("    (run H8SWEI_Snow.R with scf_method = \"2\" to produce SSPI-1)\n")
  if (any(grepl("^ssmi_results_seasonal", missing)))
    cat("    (run H1SSI_SSMI_ERALand.R to produce SSMI-1)\n")
  stop("Missing required NetCDFs. Run upstream scripts first.")
}
cat("✓ All input files exist (SPI-1/SPEI-1/SPI-3/SPEI-3/SSPI-1/SSMI-1).\n")
...
# --- 2b. SSPI-1 range check (zero-inflated gamma, clipped at ±5) -----------
sspi1_jan <- rast(files[5])
cat(sprintf("SSPI-1 Layers: %d | Cells: %d\n", nlyr(sspi1_jan), ncell(sspi1_jan)))
st_sspi <- global(sspi1_jan, c("min","max"), na.rm = TRUE)
if (max(st_sspi$max) > 5 || min(st_sspi$min) < -5) {
  warning("⚠ SSPI-1 values outside expected [-5, 5] range.")
} else {
...
# --- 2d. Grid consistency check across all six indices ----------------------
if (!isTRUE(tryCatch(compareGeom(spi1_jan, sspi1_jan, stopOnError = FALSE), error = function(e) FALSE)))
  cat("⚠ SSPI-1 grid differs from SPI-1 — A03_saf_core_pipeline.R will resample it onto the SPI-1 grid.\n")
if (!isTRUE(tryCatch(compareGeom(spi1_jan, ssmi1_jan, stopOnError = FALSE), error = function(e) FALSE)))
  cat("⚠ SSMI-1 grid differs from SPI-1 — A03_saf_core_pipeline.R will resample it onto the SPI-1 grid.\n")
