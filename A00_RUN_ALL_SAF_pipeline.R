...
#             SSPI-1 (snow), SSMI-1 (soil moisture)
...
quick_diagnostic <- function() {
  ...
  if (length(dirs) == 0) {
    cat("  No analysis folders found. Run A03_saf_core_pipeline.R first.\n")
...
#---------------------------------------------------------------------------
## STEP 2 — Main SAF pipeline
#   Defines all core functions AND immediately executes for all 6 indices.
#   On completion the following objects exist in the workspace:
#     res_spi1, res_spei1, res_spi3, res_spei3, res_sspi1, res_ssmi1
#   Each is a list with: dc, events, rp, marginals, copulas,
#                        saf_conditional, saf_kendall, saf_cond_by_class,
#                        saf_kend_by_class
# ---------------------------------------------------------------------------
cat("\n", paste(rep("=", 70), collapse=""), "\n")
cat("STEP 3: Main SAF pipeline (SPI-1, SPEI-1, SPI-3, SPEI-3, SSPI-1, SSMI-1)\n")
cat(paste(rep("=", 70), collapse=""), "\n")
source("A03_saf_core_pipeline.R")
#---------------------------------------------------------------------------
# STEP 3 — Utility / methodology helper functions (no execution, safe to load first)
# ---------------------------------------------------------------------------
cat("\n", paste(rep("=", 70), collapse=""), "\n")
cat("STEP 2: Loading methodology helper functions\n")
cat(paste(rep("=", 70), collapse=""), "\n")
source("A02_exploratory_utilities.R")
...
cat(paste(rep("=", 70), collapse=""), "\n")
source("A04_copula_dependence_diagnostics.R")
source("A05_nonstationary_saf_event_placement.R")
# ext3: regional L-moments pooling for D13+ events (requires lmomco; see
# ext3's header comment for the donor-basin CSV format it expects).
source("A06_regional_lmoments_pooling.R")
...
# Helper: named list of all six index results with their output directories
INDEX_RESULTS <- list(
  ...
  SSPI1 = list(res = res_sspi1, dir = "spatial_drought/SSPI1_analysis", scale = 1),
  SSMI1 = list(res = res_ssmi1, dir = "spatial_drought/SSMI1_analysis", scale = 1)
)
...
for (nm in names(INDEX_RESULTS)) {
  ...
  if (!is.null(gof) && gof$decision == "REJECTED") {
    plot_copula_diagnostic(u_mat, .fix_tcopula_df(res$copulas$best_copula_fit@copula), nm, out_dir)
...
# ---------------------------------------------------------------------------
# STEP 5f — Regional L-moments pooling for D13+ (long-duration) events
# Only runs for indices/classes where the main pipeline (Step 3, Sec 8e of
# SAF_Analysis.R) skipped per-class SAF fitting because n_events < 15.
# In the last verified run this was SPI3 (n=9) and SPEI3 (n=11).
#
# REQUIRES DONOR BASIN DATA the analyst must supply: run (or otherwise
# obtain) this pipeline's Step 3 event-extraction for each donor basin and
# point DONOR_BASIN_FILES at the resulting *_duration_classified_events.csv
# files. If a path doesn't exist, that donor is skipped with a logged
# warning (see ext3's load_donor_basin_events()) rather than erroring out;
# with too few usable sites (<3 total incl. Nechako) pooling is skipped
# entirely and this block just logs why.
# ---------------------------------------------------------------------------
cat("\n", paste(rep("=", 70), collapse=""), "\n")
cat("STEP 5f: Regional L-moments pooling for D13+ events\n")
cat(paste(rep("=", 70), collapse=""), "\n")

DONOR_BASIN_FILES <- list(
  Thompson         = "donor_basins/Thompson_duration_classified_events.csv",
  Chilcotin        = "donor_basins/Chilcotin_duration_classified_events.csv",
  PeaceHeadwaters  = "donor_basins/PeaceHeadwaters_duration_classified_events.csv"
)
...
for (nm in names(INDEX_RESULTS)) {
  entry <- INDEX_RESULTS[[nm]]
  res   <- entry$res
  if (is.null(res) || is.null(res$events) || nrow(res$events) == 0) next
  
  n_d13 <- sum(res$events$duration_class == "D13+ (Long-term)", na.rm = TRUE)
  if (n_d13 >= 15 || n_d13 == 0) {
    # Either the at-site sample was already sufficient (no pooling needed)
    # or there are no D13+ events at all for this index (nothing to pool).
    next
  }
  
  cat(sprintf("\n--- %s: D13+ n=%d (< 15) -- attempting regional pooling ---\n", nm, n_d13))
  
  # Optional non-stationary index flood: reuse this index's GAMLSS period
  # means (ns$mu_ref / ns$mu_rec) as a proxy for how the D13+ index flood
  # itself may have shifted, since the D13+ sample alone is too small to
  # fit its own trend. This is the hybrid non-stationary index-flood
  # extension described in ext3.R's header -- an explicit, documented
  # approximation, not a literal re-fit on D13+ data alone.
  ns_this <- enh_results[[nm]]$ns_marginals
  ns_flood <- if (!is.null(ns_this)) {
    list(reference = ns_this$mu_ref, recent = ns_this$mu_rec)
  } else NULL
  
  regional_results[[nm]] <- regional_D13_SAF(
    index_name                = nm,
    nechako_events             = res$events,
    donor_csv_list             = DONOR_BASIN_FILES,
    marginal_fits              = res$marginals,
    copula_fit_obj             = res$copulas,
    out_dir                    = entry$dir,
    nonstationary_index_flood  = ns_flood)
}
...
# 6e) Place the 2022-2025 mega-drought on the SAF surface, stationary AND
#     non-stationary (recent-epoch), peak-month AND event-mean
#     characterisation, across all six indices. This is the bivariate
#     analogue of the univariate "effective return period compression"
#     finding reported in the companion manuscripts (see ext2.R's
#     place_event_on_saf_v2() header for full rationale).
#
#     The original place_event_on_saf() (stationary, peak-month only) is
#     left available above/in ext2.R for backward compatibility, e.g.:
...
event_placement_results <- list()
for (nm in names(INDEX_RESULTS)) {
  entry <- INDEX_RESULTS[[nm]]
  res   <- entry$res
  if (is.null(res) || is.null(res$copulas) || is.null(res$marginals)) next
  
  event_placement_results[[nm]] <- place_event_on_saf_v2(
    drought_data      = res$dc,
    copula_fit_obj    = res$copulas,
    marginal_fits     = res$marginals,
    event_start        = "2022-01-01",
    event_end          = "2025-12-01",
    index_name         = nm,
    output_dir         = entry$dir,
    ns_result          = enh_results[[nm]]$ns_marginals,
    tv_copula_result   = enh_results[[nm]]$tv_copula,
    period_label       = "recent")
}
...
save_pub <- function(plot_obj, name, width = 10, height = 7, dpi = 300) {
  ...
  render_it <- function() {
    if (inherits(plot_obj, "gg") || inherits(plot_obj, "patchwork")) {
      print(plot_obj)
    } else if (inherits(plot_obj, c("gtable", "grob", "gTree"))) {
      # grid.arrange() returns a gtable — draw it directly.
      # Must NOT pass a gtable as grobs= to grid.arrange(); that miscounts.
      grid::grid.newpage()
...
# Fig 1: Monthly time series of severity and area_pct for all 6 indices ------
{
  ...
  fig01 <- p1a / p1b +
    plot_annotation(
      ...
      subtitle = "Threshold = −0.5  |  SPI-1, SPEI-1, SPI-3, SPEI-3, SSPI-1, SSMI-1",
      theme    = pub_theme()
...
# Fig 4: Severity — empirical vs best-fit marginal (4-panel) -----------------
{
  ...
  fig04  <- wrap_plots(panels, ncol = 3) +
...
# Fig 5: Area — empirical vs Beta fit (4-panel) ------------------------------
{
  ...
  panels <- panels[!sapply(panels, is.null)]
  fig05  <- wrap_plots(panels, ncol = 3) +
...
# Fig 6: Pseudo-observation scatter with fitted copula contours (4-panel) ----
{
  ...
  panels <- panels[!sapply(panels, is.null)]
  fig06  <- wrap_plots(panels, ncol = 3) +
...
# Fig 7: Kendall τ summary barplot + class breakdown -------------------------
{
  ...
  fig07 <- ggplot(tau_df, aes(x = Index, y = tau, fill = Index)) +
    ...
    geom_text(aes(label = sprintf("tau=%.3f\nrho=%.3f", tau, rho)),
              vjust = -0.5, size = 3.5) +
    scale_fill_manual(values = INDEX_COLOURS) +
...
# Fig 8: Copula AIC comparison (grouped bar) ---------------------------------
{
  ...
  if (nrow(aic_df) > 0 && "AIC" %in% names(aic_df) && "Copula" %in% names(aic_df)) {
    ...
    fig08 <- ggplot(aic_df, aes(x = Copula, y = delta_AIC, fill = Index)) +
      ...
      labs(x = "Copula Family", y = "dAIC (relative to best)",
           title = "Copula Model Selection by Index",
           subtitle = "Lower dAIC = better fit  |  Best model at dAIC = 0") +
      pub_theme() +
...
# Fig 11: Conditional vs Kendall, 4-panel grid --------------------------------
{
  ...
  panels <- panels[!sapply(panels, is.null)]
  fig11  <- wrap_plots(panels, ncol = 3) +
    plot_annotation(
...
# Fig 12: Per-class SAF curves for all indices --------------------------------
{
  ...
  # NOTE: originally hand-picked SPI1/SPI3 only; now generalized to loop over
  # every index in INDEX_RESULTS (SPI1, SPEI1, SPI3, SPEI3, SSPI1, SSMI1) so
  # the two new indices are included automatically without further edits here.
  p12_panels <- lapply(names(INDEX_RESULTS), make_class_saf)
  names(p12_panels) <- names(INDEX_RESULTS)
  p12_panels <- p12_panels[!sapply(p12_panels, is.null)]
  if (length(p12_panels) > 0) {
    fig12 <- wrap_plots(p12_panels, ncol = 3) +
      plot_annotation(title = "SAF Curves by Duration Class: All Indices",
                      theme = pub_theme())
    save_pub(fig12, "Fig12_SAF_by_duration_class", width = 14, height = 10)
    cat("  Fig12 saved.\n")
  } else {
...
# Fig 13: NS SAF — reference vs recent, conditional, all 6 indices -----------
{
  ...
  panels <- panels[!sapply(panels, is.null)]
  if (length(panels) > 0) {
    fig13 <- wrap_plots(panels, ncol = 3) +
      plot_annotation(
...
# Fig 17: Rosenblatt PIT diagnostic — dynamic N-panel (save JPEG like the attached) --
{
  plot_rosenblatt <- function(nm) {
    ...
    uS    <- res$copulas$u_severity
    uA    <- res$copulas$u_area
    cop   <- .fix_tcopula_df(res$copulas$best_copula_fit@copula)  # FIX: pCopula needs integer df
  ...
  # FIX: the old code only combined panels when length(valid_panels) == 4,
  # via a hardcoded 2x2 (a|b)/(c|d) layout. With 6 indices now producing
  # valid copulas (post CRS-fix, SSPI1 included), that condition never
  # fires and the combined figure silently never gets built, despite the
  # "Fig17 saved (individual + combined)" message claiming it did.
  #
  # Generalized: any number of valid panels (1 to N) now combines via
  # wrap_plots(ncol = 3), matching the layout convention already used by
  # Fig11/Fig12/Fig13/Fig18 elsewhere in this script, instead of a
  # count-specific manual grid.
  valid_panels <- panels_rob[!sapply(panels_rob, is.null)]
  if (length(valid_panels) > 0) {
    fig17_comb <- wrap_plots(valid_panels, ncol = 3) +
      plot_annotation(title = "Rosenblatt PIT Diagnostic — All Indices",
                      theme = pub_theme())
    # Scale figure size to panel count so a 6-panel combo isn't
    # squeezed into dimensions sized for 4.
    n_rows <- ceiling(length(valid_panels) / 3)
    save_pub(fig17_comb, "Fig17_rosenblatt_combined",
             width = 18, height = 7 * n_rows)
    cat(sprintf("  Fig17 saved (individual x%d + combined, %d-panel).\n",
                length(valid_panels), length(valid_panels)))
  } else {
    cat("  Fig17 skipped (no valid copula panels).\n")
  }
}
...
# Fig 18: Rolling Kendall τ, 4-panel ------------------------------------------
{
  ...
  if (length(rt_plots) > 0) {
    fig18 <- wrap_plots(rt_plots, ncol = 3) +
      plot_annotation(
...
# Table 1: Return periods consolidated ---------------------------------------
{
  ...
  headerStyle <- createStyle(fontColour = "#FFFFFF", fgFill = "#2c5f8a",
                             ...
                             border = "Bottom", textDecoration = "bold")
