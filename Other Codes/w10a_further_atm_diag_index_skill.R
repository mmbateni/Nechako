...
#   • NOAA climate index flat files (PDO, PNA, AMO, Nino3.4, AO)
#
...
# Independent of w10a — can run concurrently.
# ==============================================================================
...
# ------------------------------------------------------------------------------
# LOAD CLIMATE INDICES (flat NOAA files)
# ------------------------------------------------------------------------------
cat("\nLoading climate indices from NOAA files...\n")

load_noaa_index <- function(filepath, col_name, skip_rows = 1) {
  if (!file.exists(filepath)) return(NULL)
  raw <- read.table(filepath, header = FALSE, skip = skip_rows, fill = TRUE,
                    na.strings = c("-99.99","-9.99","99.99","999.9"))
  colnames(raw) <- c("year", month.abb)
  raw %>%
    filter(!is.na(year), year >= 1950, year <= 2025) %>%
    pivot_longer(cols = all_of(month.abb), names_to = "month_name", values_to = col_name) %>%
    mutate(month_num = match(month_name, month.abb),
           date = as.Date(sprintf("%d-%02d-01", year, month_num))) %>%
    select(date, year, month_num, all_of(col_name))
}

idx_dir  <- "climate_indices"
idx_list <- Filter(Negate(is.null), list(
  load_noaa_index(file.path(idx_dir,"pdo_index.txt"),    "PDO"),
  load_noaa_index(file.path(idx_dir,"pna_index.txt"),    "PNA"),
  load_noaa_index(file.path(idx_dir,"amo_index.txt"),    "AMO"),
  load_noaa_index(file.path(idx_dir,"nino34_index.txt"), "Nino34"),
  load_noaa_index(file.path(idx_dir,"ao_index.txt"),     "AO")
))

if (!length(idx_list)) {
  cat("WARNING: No climate index files found – B2 skipped.\n")
  quit(save = "no")
}
