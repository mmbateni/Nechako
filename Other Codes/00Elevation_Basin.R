...
if (has_bcdata) {
  tryCatch({
    ...
    rivers_sf <- bcdc_query_geodata("freshwater-atlas-stream-network") |>
      filter(STREAM_ORDER >= 5) |>  # lowered from 6: captures Nechako main stem near Kenney Dam
...
if (nrow(rivers_sf) == 0) {
  ...
  rivers_sf <- rivers_sf |>
    mutate(line_width = case_when(
      ...
      STREAM_ORDER >= 6 ~ 0.50, STREAM_ORDER >= 5 ~ 0.35, TRUE ~ 0.25))
} else {
  rivers_sf$line_width <- 0.5
}


# ── 5.45 BC DAMS (authoritative infrastructure coordinates) ──────────────────
# Query the BC Dams catalogue layer so infrastructure markers sit on the actual
# river/structure footprint rather than relying on hardcoded coordinates.
# Results feed into section 7b where kenney_lon/lat are resolved.
cat("── Fetching BC Dams from catalogue ──\n")
bc_dams_catalogue_sf <- NULL
if (has_bcdata) {
  bc_dams_catalogue_sf <- tryCatch({
    bcdc_query_geodata("bc-dams") |>
      filter(INTERSECTS(basin_3005_query)) |>
      select(DAM_NAME, SPILLWAY_CAPACITY) |>
      collect() |>
      st_transform(4326)
  }, error = function(e) {
    message("  BC Dams catalogue query failed: ", e$message)
    NULL
  })
  if (!is.null(bc_dams_catalogue_sf))
    cat(sprintf("  BC Dams in basin: %d\n", nrow(bc_dams_catalogue_sf)))
}
...
# Kenney Dam coordinates: resolved from bc_dams_catalogue_sf (section 5.45)
# if available; otherwise fall through to a targeted query; last resort uses
# corrected hardcoded coords (old value -125.020/53.730 was ~2 km off the river).
kenney_lon <- -124.996   # corrected fallback
kenney_lat <-   53.715

if (!is.null(bc_dams_catalogue_sf) && nrow(bc_dams_catalogue_sf) > 0) {
  # Use the already-fetched catalogue result (free — no extra network call)
  kenney_row <- bc_dams_catalogue_sf[
    grepl("kenney", bc_dams_catalogue_sf$DAM_NAME, ignore.case = TRUE), ]
  if (nrow(kenney_row) > 0) {
    coords     <- st_coordinates(kenney_row[1, ])
    kenney_lon <- coords[1, "X"]
    kenney_lat <- coords[1, "Y"]
    cat(sprintf("  Kenney Dam from pre-fetched catalogue: %.5f, %.5f\n",
                kenney_lon, kenney_lat))
  }
} else if (has_bcdata) {
  # Fallback: targeted query (only runs if section 5.45 failed entirely)
  tryCatch({
    cat("  Snapping Kenney Dam via targeted bcdata query ...\n")
    dams_bc <- bcdc_query_geodata("bc-dams") |>
      filter(DAM_NAME %LIKE% "%Kenney%") |>
      select(DAM_NAME) |>
      collect() |> st_transform(4326)
    if (nrow(dams_bc) > 0) {
      coords     <- st_coordinates(dams_bc[1, ])
      kenney_lon <- coords[1, "X"]
      kenney_lat <- coords[1, "Y"]
      cat(sprintf("  Kenney Dam from targeted query: %.5f, %.5f\n",
                  kenney_lon, kenney_lat))
    } else {
      cat("  Kenney Dam not found — using corrected fallback coords.\n")
    }
  }, error = function(e) {
    message("  BC Dams query failed: ", e$message, " — using corrected fallback.")
  })
}

infra_df <- data.frame(
  name    = c("Kenney Dam",  "Skins Lake Spillway", "Kemano Powerhouse"),
  lon     = c(kenney_lon,    -125.970,               -127.882),
  lat     = c(kenney_lat,      53.771,                  53.558),
...
# ── 7c. Combined label data frame (municipalities + Prince George only)
# Infrastructure sites are identified by symbols in the legend — no map labels.
