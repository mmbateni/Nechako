...
#
# NOTES:
#   [QuickStart] total_events now sourced from nrow(ev_file) when the raw 
#   event CSV exists (authoritative); sum(rp$n_events) retained as fallback 
#   and cross-check target. MISMATCH flag compares the two counts.
#
...
#   FIX-1F  Fig 14 / Table 3 cp_year extraction corrected: ns$cp_year does 
#           not exist in fit_nonstationary_marginals() return value; cp_year 
#           is now parsed from ns$epoch_src (consistent with .resolve_epoch() 
#           in ext2).
...
# FIX-5: Override quick_diagnostic() to search "spatial_drought" instead of 
# the hardcoded "drought_analysis" path in QuickStart.R.
quick_diagnostic <- function() {
  ...
  for (d in sort(dirs)) {
    ...
    for (i in seq_len(nrow(rp))) {
      ...
      saf_fl <- if (n_ev >= 15) " " else "  [SAF skipped: n <15]"
      
      cat(sprintf("  %-30s  %8d  %12s  %10s  %10s%s\n",
                  r$duration_class, n_ev, rp_yr, ms, ma, saf_fl))
    }
...
# ============================================================================
# STEP 2 — Utility / methodology helper functions
# ============================================================================
cat("\n", paste(rep("=", 70), collapse = " "), "\n")
...
# ============================================================================
# STEP 4 — Load extension functions
# ============================================================================
cat("\n", paste(rep("=", 70), collapse = " "), "\n")
cat("STEP 4: Loading enhancement functions (ext1 v6 + ext2 v5)  # FIX-8\n")
cat(paste(rep("=", 70), collapse = " "), "\n")
...
for (nm in names(INDEX_RESULTS)) {
  ...
  tv <- fit_timevarying_copula(res$dc, res$copulas, res$marginals, year_indices, nm, out_dir)
  
  # ------------------------------------------------------------------------
  #   DIAGNOSTICS (runs after GOF & TV fit)
  # ------------------------------------------------------------------------
    # --- Copula contour diagnostic (monthly data acceptable for contour visual) ---
  ...
  # --- Rosenblatt PIT diagnostic: USE EVENT-LEVEL DATA TO REMOVE TEMPORAL ARC ---
  # Monthly snapshots are intra-event clustered. PIT on events restores i.i.d. assumption.
  if (!is.null(res$events) && nrow(res$events) > 5) {
    ev      <- res$events
    n_ev    <- nrow(ev)
    
    # Event-level pseudo-observations
    uS_ev   <- rank(ev$max_severity, ties.method = "average") / (n_ev + 1)
    uA_ev   <- rank(ev$max_area_pct,  ties.method = "average") / (n_ev + 1)
    
    cop_ev  <- .fix_tcopula_df(res$copulas$best_copula_fit@copula)
    fam_ev  <- res$copulas$best_copula_name
    
    # Compute e2 = C(uA | uS) on independent events
    e2_vals_ev <- vapply(seq_len(n_ev), function(i) {
      tryCatch(.h_func_fd(uS_ev[i], uA_ev[i], cop_ev, cop_family = fam_ev),
               error = function(e) NA_real_)
    }, numeric(1))
    
    valid_ev <- is.finite(e2_vals_ev) & is.finite(uS_ev)
    plot_rosenblatt_diagnostic(uS_ev[valid_ev], e2_vals_ev[valid_ev], nm, out_dir)
  }
  enh_results[[nm]] <- list(dependency = dep,gof = gof,tv_copula  = tv)
  }
...
INDEX_COLOURS <- c(SPI1 = "#1b7837", SPEI1 = "#762a83", SPI3 = "#2166ac", SPEI3 = "#d6604d")
...
# Fig 1: Monthly time series of severity and area_pct for all 4 indices
{
  ...
  p1a <- ggplot(all_dc, aes(x = date, y = severity, colour = Index)) +
    ...
    labs(x = NULL, y = "Mean Drought Severity", title = NULL, colour = "Index") +
    pub_theme()
...
# Fig 2: Event scatter — severity vs area, coloured by duration class
{
  ...
  fig02 <- ggplot(all_ev, aes(x = mean_area_pct, y = mean_severity,
    ...
    labs(x = "Mean Drought-Affected Area (%)", y = "Mean Drought Severity",
...
# Fig 4: Severity — empirical vs best-fit marginal (4-panel)
{
  plot_severity_marginal <- function(nm) {
    ...
    ggplot(dc, aes(x = severity)) +
      ...
      geom_line(data = df_fit, aes(x = x, y = y), colour = "black", linewidth = 1.1) +
  ...
  fig04  <- wrap_plots(panels, ncol = 2) +
    plot_annotation(title = "Marginal Distribution Fit: Drought Severity", theme = pub_theme())
...
# Fig 5: Area — empirical vs Beta fit (4-panel)
{
  ...
  fig05  <- wrap_plots(panels, ncol = 2) +
    plot_annotation(title = "Marginal Distribution Fit: Drought-Affected Area", theme = pub_theme())
...
# Fig 6: Pseudo-observation scatter with fitted copula contours (4-panel)
{
  plot_pseudo_obs <- function(nm) {
    ...
    # FIX-1E: dep$kendall_tau → dep$kendall
    ggplot() +
      geom_contour(data = grid, aes(x = u1, y = u2, z = density),
                   colour = "steelblue", bins = 10, linewidth = 0.5, alpha = 0.7) +
      geom_point(aes(x = uS, y = uA), size = 1.4, alpha = 0.5, colour = INDEX_COLOURS[nm]) +
      labs(title = nm,
           subtitle = sprintf("Best copula: %s  |  τ=%.3f", best_name, enh_results[[nm]]$dependency$kendall),
  ...
  fig06  <- wrap_plots(panels, ncol = 2) +
    plot_annotation(title = "Pseudo-Observations and Fitted Copula Contours", theme = pub_theme())
...
# Fig 8: Copula AIC comparison (grouped bar)
{
  ...
  if (nrow(aic_df) > 0 && "AIC" %in% names(aic_df) && "Copula" %in% names(aic_df)) {
    ...
    fig08 <- ggplot(aic_df, aes(x = Copula, y = delta_AIC, fill = Index)) +
      ...
      pub_theme() + theme(axis.text.x = element_text(angle = 30, hjust = 1))
...
# Fig 9 & 10: SAF at specific return periods — all indices on one panel
{
  make_saf_allindices <- function(rp_filter, method = "Conditional", fig_name) {
    ...
    if (nrow(all_saf) == 0) {
      cat(sprintf("  %s skipped — no data.\n", fig_name))
      return(NULL)
    }
    
    ggplot(all_saf, aes(x = Area_pct, y = Severity, colour = Index, linetype = Index)) +
...
# Fig 11: Conditional vs Kendall, 4-panel grid
{
  plot_method_comp <- function(nm) {
    ...
    comb <- rbind(cond[is.finite(cond$Severity), ], kend[is.finite(kend$Severity), ])
    
    ggplot(comb, aes(x = Area_pct, y = Severity,
                     colour = factor(ReturnPeriod_years), linetype = Method)) +
      geom_line(linewidth = 1.0) +
      scale_colour_manual(values = RP_COLOURS, name = "RP (yr)") +
      scale_linetype_manual(values = c(Conditional = "solid", Kendall = "dashed"), name = "Method") +
...
# Fig 12: Per-class SAF curves for SPI-1 and SPI-3
{
  make_class_saf <- function(nm, method = "Conditional") {
    ...
    ggplot(all_cls, aes(x = Area_pct, y = Severity,
                        colour = factor(ReturnPeriod_years), linetype = DurationClass)) +
      geom_line(linewidth = 1.0) +
      scale_colour_manual(values = RP_COLOURS, name = "RP (yr)") +
      scale_linetype_discrete(name = "Duration Class") +
      labs(title = nm, x = "Drought-Affected Area (%)", y = "Drought Severity") +
      pub_theme(base_size = 11)
  }
  ...
  if (!is.null(p12a) && !is.null(p12b)) {
    fig12 <- p12a / p12b +
      plot_annotation(title = "SAF Curves by Duration Class: SPI-1 and SPI-3", theme = pub_theme())
...
# Fig 13: NS SAF — reference vs recent, conditional, all 4 indices
{
  plot_ns_comp <- function(nm) {
    ...
    ns_ref <- enh$saf_ns_ref
    ns_rec <- enh$saf_ns_rec
    ns_ref$Epoch <- "Reference"
    ns_rec$Epoch <- "Recent"
    comb <- rbind(ns_ref[is.finite(ns_ref$Severity), ], ns_rec[is.finite(ns_rec$Severity), ])
    if (nrow(comb) == 0) return(NULL)
    
    ggplot(comb, aes(x = Area_pct, y = Severity,
                     colour = factor(ReturnPeriod_years), linetype = Epoch)) +
      geom_line(linewidth = 1.0) +
      scale_colour_manual(values = RP_COLOURS, name = "RP (yr)") +
      scale_linetype_manual(values = c(Reference = "solid", Recent = "dashed"), name = "Epoch") +
      labs(title = nm, x = "Area (%)", y = "Severity") +
      pub_theme(base_size = 11)
  }
...
{
  # FIX-1F: Parse cp_year from ns$epoch_src
  extract_cp_year_from_ns <- function(ns) {
    if (is.null(ns) || !grepl("change-point", ns$epoch_src, fixed = TRUE)) return(NA_integer_)
  ...
  if (nrow(ns_aic_df) > 0) {
    ...
    fig14 <- gridExtra::grid.arrange(
      ...
      heights = grid::unit.c(grid::unit(1, "line"), grid::unit(1, "null"), grid::unit(1, "line"))
    )
...
# Fig 16: Change-point detection summary
{
  cp_df <- dplyr::bind_rows(lapply(names(INDEX_RESULTS), function(nm) {
    ...
    # FIX-1B: Correct field names from detect_copula_changepoints()
    data.frame(
      Index    = nm,
...
# Fig 17: Rosenblatt PIT diagnostic — 4-panel
{
  plot_rosenblatt <- function(nm) {
    ...
    ev  <- res$events
    n_e <- nrow(ev)
    uS  <- rank(ev$max_severity, ties.method = "average") / (n_e + 1)
    uA  <- rank(ev$max_area_pct, ties.method = "average") / (n_e + 1)
    cop  <- .fix_tcopula_df(res$copulas$best_copula_fit@copula)
    best <- res$copulas$best_copula_name
    
    e1 <- uS
    e2 <- .h_func_fd(uS, uA, cop, cop_family = best)
    e2 <- pmax(pmin(e2, 1), 0)
    ...
    p_hist <- ggplot(df_e2, aes(x = e2)) +
      geom_histogram(aes(y = after_stat(density)), bins = 15, fill = "#6baed6", colour = "white") +
  ...
  for (nm in names(panels_rob)) {
    if (!is.null(panels_rob[[nm]])) {
      ...
      print(panels_rob[[nm]])
      dev.off()
      save_pub(panels_rob[[nm]], sprintf("Fig17_%s_rosenblatt", nm), width = 9, height = 7)
    }
  }
  ...
  if (length(valid_panels) == 4) {
    fig17_comb <- (valid_panels[[1]] | valid_panels[[2]]) /
      ...
      plot_annotation(title = "Rosenblatt PIT Diagnostic — All Indices", theme = pub_theme())
...
# Fig 18: Rolling Kendall τ, 4-panel
{
  compute_rolling_tau <- function(dc, window = 10) {
    ...
    for (i in seq_len(n - window + 1)) {
      ...
      out_tau[i] <- if (sd(s_w) == 0 || sd(a_w) == 0) NA_real_ else cor(s_w, a_w, method = "kendall")
  ...
  rt_plots <- lapply(names(INDEX_RESULTS), function(nm) {
    ...
    # FIX-1D: cp_result$cp_year instead of consensus_yr
    cp_yr <- tryCatch(enh_results[[nm]]$tv_copula$cp_result$cp_year, error = function(e) NA)
    
    ggplot(rt, aes(x = year, y = tau)) +
...
# Table 2: Copula fit summary
{
  ...
  if (nrow(cop_all) > 0) {
    ...
    if (length(best_rows) > 0) {
      ...
      addStyle(wb2, 1, bestStyle, rows = best_rows, cols = seq_len(ncol(cop_all)), gridExpand = TRUE)
    }
...
{
  # Reuse helper for cp_year parsing (FIX-1F) & fix significance field (FIX-1C)
  extract_cp_year_from_ns <- function(ns) {
    if (is.null(ns) || !grepl("change-point", ns$epoch_src, fixed = TRUE)) return(NA_integer_)
    m <- regmatches(ns$epoch_src, regexpr("[0-9]{4}", ns$epoch_src))
    if (length(m) == 1L) as.integer(m) else NA_integer_
  ...
  ns_all <- dplyr::bind_rows(lapply(names(INDEX_RESULTS), function(nm) {
    ...
    data.frame(
      Index         = nm,
      CP_year       = extract_cp_year_from_ns(ns),
