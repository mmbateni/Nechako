...
#   (1) Loads BOTH PET inputs produced by 2b_PET_ERALand.R:
...
#   (7) [NEW — Counterfactual Branch] Runs TWO additional SPEI calculations
#       from month-specific linearly detrended PET inputs (Script 2b/2c):
#       RUN 3: wb_det_pm_basin  -> spei_results_detrended/     (SPEI_PM_det)
#       RUN 4: wb_det_thw_basin -> spei_results_detrended_thw/ (SPEI_Thw_det)
#       Precipitation remains at OBSERVED values in all four runs.
#       RUN_DETRENDED_BRANCH flag controls whether Runs 3/4 execute;
#       set to FALSE to skip without affecting the observed outputs.
#
# The two observed SPEI variants enable Dynamic vs. Thermodynamic decomposition:
...
#
# The detrended pair adds a counterfactual dimension:
#   SPEI_PM_det  = SPEI with warming trend removed from PET
#   SPEI_Thw_det = temperature-only thermodynamic component, no warming trend
#   SPEI_PM - SPEI_PM_det ~ warming contribution to drought severity
##############################################
...
# [NEW] Counterfactual output directories (detrended T branch)
...
# [NEW] Set to TRUE to run counterfactual SPEI (Runs 3 & 4) using detrended
# PET from 2b_PET_ERALand.R Section 5c (requires 2b_detrend_temperature.R).
# Set to FALSE to run only the observed branch (no change to existing outputs).
...
# [NEW] Detrended PET inputs (counterfactual branch, from Section 5c of Script 2)
if (RUN_DETRENDED_BRANCH) {
  f_pet_det     <- "monthly_data_direct/potential_evapotranspiration_PM_detrended.nc"
  f_pet_thw_det <- "monthly_data_direct/potential_evapotranspiration_Thw_detrended.nc"
  if (!file.exists(f_pet_det) || !file.exists(f_pet_thw_det)) {
    cat("  WARNING: Detrended PET files not found — disabling counterfactual runs.\n")
    cat(sprintf("  Expected: %s\n", f_pet_det))
    cat(sprintf("  Expected: %s\n", f_pet_thw_det))
    cat("  Run 2a_detrend_temperature.R then 2b_PET_ERALand.R (RUN_DETRENDED_BRANCH=TRUE) first.\n")
    RUN_DETRENDED_BRANCH <- FALSE   # graceful fallback
  } else {
    pet_det     <- rast(f_pet_det)
    pet_thw_det <- rast(f_pet_thw_det)
    cat("PM  PET (detrended T) loaded\n")
    cat("Thw PET (detrended T) loaded\n")
  }
}
##############################################
...
# [NEW] Align detrended PET grids if the counterfactual branch is active
if (RUN_DETRENDED_BRANCH) {
...
if (RUN_DETRENDED_BRANCH) {
  ...
  cat("Converting WB_det (PM) from mm/day to mm/month...\n")
  for (i in seq_len(nlyr(wb_det))) wb_det[[i]] <- wb_det[[i]] * days_in_month[i]
  terra::time(wb_det) <- dates
  
  wb_det_mat   <- values(wb_det, mat = TRUE)
  wb_det_basin <- wb_det_mat[basin_mask, , drop = FALSE]
  cat(sprintf("Basin pixels for WB_det (PM): %d\n", nrow(wb_det_basin)))
  
  cat("\n===== WATER BALANCE (Thornthwaite detrended) =====\n")
  wb_det_thw <- precip - pet_thw_det
  cat("Converting WB_det (Thw) from mm/day to mm/month...\n")
  ...
  cat(sprintf("Basin pixels for WB_det (Thw): %d\n", nrow(wb_det_thw_basin)))
  
  cat(sprintf("  WB_det PM  mean=%.2f, sd=%.2f mm/month\n",
              mean(wb_det_basin,     na.rm = TRUE), sd(wb_det_basin,     na.rm = TRUE)))
  cat(sprintf("  WB_det Thw mean=%.2f, sd=%.2f mm/month\n",
              mean(wb_det_thw_basin, na.rm = TRUE), sd(wb_det_thw_basin, na.rm = TRUE)))
  cat("=========================================\n\n")
}
...
run_spei_loop <- function(wb_input, target_dir, pet_label,
  ...
  for (scale in scales) {
    ...
    method_full[basin_mask] <- method_basin[, 24]
...
# ==============================================================================
#   RUN 3: SPEI_PM_det  (PM PET, detrended T — counterfactual)  [NEW]
# ==============================================================================
if (RUN_DETRENDED_BRANCH) {
  cat("\n##############################################################\n")
  cat("  RUN 3 OF 4 [NEW]: SPEI_PM_det  (PM PET, detrended T)\n")
  cat("  Precipitation: OBSERVED (unchanged)\n")
  cat("  PET: Penman-Monteith computed from month-specific detrended T\n")
  ...
  run_spei_loop(
    ...
    pet_label         = "_PM_det",
    cl_in             = cl,
    wb_template       = wb_det[[1]],
    summary_file_path = file.path(out_dir_det, "spei_det_all_scales_summary.txt")
  )
}

# ==============================================================================
#   RUN 4: SPEI_Thw_det  (Thornthwaite PET, detrended T — counterfactual)  [NEW]
# ==============================================================================
if (RUN_DETRENDED_BRANCH) {
  cat("\n##############################################################\n")
  cat("  RUN 4 OF 4 [NEW]: SPEI_Thw_det  (Thw PET, detrended T)\n")
  cat("  Precipitation: OBSERVED (unchanged)\n")
  cat("  PET: Thornthwaite computed from month-specific detrended T\n")
...
cat("\n============================================================\n")
if (RUN_DETRENDED_BRANCH) {
  cat("SPEI CALCULATION COMPLETE -- ALL 4 PET VARIANTS\n")
} else {
...
if (RUN_DETRENDED_BRANCH) {
  cat(sprintf("SPEI_PM_det   output : %s\n", normalizePath(out_dir_det)))
  cat(sprintf("SPEI_Thw_det  output : %s\n", normalizePath(out_dir_det_thw)))
}
...
if (RUN_DETRENDED_BRANCH) {
  cat("\nNEXT STEP (counterfactual) -- Warming signal isolation:\n")
  cat("  SPEI_PM - SPEI_PM_det = warming contribution to drought severity\n")
  cat("  Run 11_SPEI_counterfactual_comparison.R\n")
}
...
## ── (A + B) Assemble and (C) write basin-average CSVs for scales 1,2,3,6,12 ─────────

EXPORT_SCALES <- c(1L, 2L, 3L, 6L, 12L)
