...
create_comparison_figure <- function() {
  vars_avail <- Filter(function(v) !is.null(get(paste0("r_",v))),
  ...
  pdf_path <- file.path(out_dir, "four_variable_trend_comparison.pdf")
  result <- .pdf_safe(pdf_path, width=5*n, height=6, expr_fn=function() {
    par(mfrow=c(1,n), mar=c(2,1.5,2.5,1), oma=c(1,1,3,1))
    for (v in names(r_list))
      plot_raster_panel(r_list[[v]], var_labels[v], c(-0.3,0.3),
                        hcl.colors(101,"RdBu",rev=TRUE), legend_title="τ")
    mtext("Nechako Basin Annual Trends (1950-2025) | VC-MK",
          outer=TRUE, cex=1.2, font=2, line=0.5)
  })
  if (!is.null(result)) cat(sprintf("✅ Four-variable comparison map: %s\n", basename(pdf_path)))
  invisible(result)
}
...
create_statistics_table <- function() {
  ...
  stats_summary <- all_results[period=="annual" & !is_basin_average, .(
    ...
    Pct_Sig_VC    = {
      n <- sum(!filtered_vc)
      if (n>0) sprintf("%.1f%%", sum(p_value_vc<0.05 & !filtered_vc)/n*100)
      else "N/A"
    },
    Pct_Sig_TFPW  = {
      n <- sum(!filtered_tfpw)
      if (n>0) sprintf("%.1f%%", sum(p_value_tfpw<0.05 & !filtered_tfpw)/n*100)
      else "N/A"
    },
...
create_basin_timeseries_plot <- function() {
  ...
  # Panel 2: PET_PM vs PET_Thw (overlaid)
  par(mar=c(if (n_panels>2) 3 else 4, 5, 2, 2))
...
# For a given variable, shows 12 panels — one per calendar month.
# Each panel: time series of that month's value across all years with OLS trend.
# ──────────────────────────────────────────────────────────────────────────────────
####################################################################################

#' Build per-calendar-month temporal series from basin-averaged monthly vector.
#' Returns a data.frame(year, month, month_abb, value) ready for ggplot.
...
#' 12-panel calendar-monthly temporal-change figure for one variable.
#' Each panel = one calendar month; y-axis = the observed value that year;
#' OLS line drawn if n ≥ 10; significance label (* p < 0.05).
...
#' Wrapper that calls create_calmonth_temporal_changes for all 4 variables.
#' Also creates a combined 4-row slope-profile comparison (Function 13e).
create_all_calmonth_temporal_changes <- function() {
  ...
  # ── 13c PET Thornthwaite (only if available) ──────────────────────────────
  ...
  # ── 13e Combined: per-month OLS slope profile (all vars, one figure) ──────
...
create_pet_comparison_plots <- function() {
  ...
  # ── 14a: Time series overlay (monthly + 12-mo rolling mean) ───────────────
  ...
  # ── 14b: Per-month OLS slope comparison bar chart ─────────────────────────
  ...
  # ── 14c: Basin trend map comparison (Sen's slope, VC-MK) ──────────────────
  ...
  # ── 14d: Monthly bias (PM − Thw) barplot ─────────────────────────────────
  ...
  # ── 14e: Per-month scatter PM vs Thw coloured by decade ───────────────────
...
create_temperature_dedicated_plots <- function() {
  ...
  # 12b: annual anomaly barplot (vs 1991-2020)
  ...
  p12b <- ggplot2::ggplot(df_anom, ggplot2::aes(x=year, y=anomaly, fill=sign)) +
    ...
    ggplot2::geom_line(ggplot2::aes(y=run10), colour="black", linewidth=1.0, na.rm=TRUE) +
    ggplot2::scale_fill_manual(values=c(Warm="#d73027",Cool="#4575b4"), name=NULL) +
    ggplot2::geom_hline(yintercept=0, colour="grey20", linewidth=0.45) +
    ggplot2::scale_x_continuous(breaks=seq(1950,2025,by=10)) +
    ggplot2::labs(title="Annual Temperature Anomaly — Nechako River Basin",
                  subtitle=sprintf("Baseline: WMO 1991\u20132020 (%.2f\u00b0C).  Black = 10-yr mean.  %s",
                                   clim_mean, anom_str),
...
# [NEW] Calendar-monthly temporal changes (Functions 13a–13e)
create_all_calmonth_temporal_changes()

# [NEW] PET_PM vs PET_Thw comparison (Function 14)
create_pet_comparison_plots()

# Temperature dedicated
create_temperature_dedicated_plots()

# Specific-point outputs
