...
required_cols <- c("x", "y", "variable", "period", "tau_vc", "p_value_vc", 
                   "sl_vc", "tau_tfpw", "p_value_tfpw", "filtered_vc", 
                   "is_basin_average", "n_spectral_peaks", "changepoint_detected")
...
cat("📐 Creating raster template from pixel coordinates...\n")
pet_annual_sample <- all_results[variable == "PET" & period == "annual" & 
...
if (nrow(pet_annual_sample) == 0 || all(is.na(pet_annual_sample$x))) {
  ...
  if (!is.finite(dx) || !is.finite(dy) || dx <= 0 || dy <= 0) {
    resolution_bc <- 1000
    cat("⚠️  Using fallback resolution:", resolution_bc, "m\n")
  } else {
...
create_raster_from_table <- function(results_dt, template, value_col) {
  ...
  if (!value_col %in% names(results_dt)) {
    stop(sprintf("Column '%s' not found. Available: %s", 
                 value_col, paste(head(names(results_dt)), collapse = ", ")))
  }
...
if (!is.null(regime_fig)) {
  cat("  • regime_shifts_changepoints.pdf\n")
}
if (!is.null(spectral_fig)) {
  cat("  • spectral_analysis.pdf\n")
}
cat("  • spatial_patterns.pdf\n")
cat("  • method_comparison_vc_vs_tfpw.png\n")
