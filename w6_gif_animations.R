...
render_gif <- function(index_type, scale, data_dir, find_fn, basin_shp,
  ...
  # ── FIX 1: Trim initial NA layers (warm-up period for SPI/SPEI) ───────────
  cli::cli_alert_info("Checking for NA layers (warm-up period)...")
  layer_means <- terra::global(r, "mean", na.rm = TRUE)$mean
  valid_layers <- !is.na(layer_means)
  if (sum(!valid_layers) > 0) {
    cli::cli_alert_info("Trimming {sum(!valid_layers)} initial NA layers.")
    r <- r[[valid_layers]]
    dates <- dates[valid_layers]
    n <- terra::nlyr(r)
  }
  ...
  # ── FIX 2: Ensure CRS match before crop/mask ───────────────────────────────
  if (terra::crs(basin_shp) != terra::crs(r)) {
    cli::cli_alert_warning("CRS mismatch. Projecting basin to raster CRS.")
    basin_shp <- terra::project(basin_shp, terra::crs(r))
  }
  ...
  for (i in seq_len(n)) {
    # 1. Create fresh data frame for current frame
    df_frame <- data.frame(x = xy$x, y = xy$y, value = vals[, i])
    
    # 2. Build plot with current data
    p <- ggplot2::ggplot() +
      ggplot2::geom_raster(data = df_frame, mapping = ggplot2::aes(x = x, y = y, fill = value)) +
      ggplot2::geom_contour(data = df_frame, mapping = ggplot2::aes(x = x, y = y, z = value),
                            bins = 6, colour = "grey25", linewidth = 0.35, na.rm = TRUE) +
      ggplot2::geom_path(data = basin_geom, mapping = ggplot2::aes(x = x, y = y, group = group),
                         colour = "#1B5E20", linewidth = 1.2, lineend = "round") +
      scico::scale_fill_scico(palette = "vik", limits = DROUGHT_LIMITS, midpoint = 0, name = title_str,
                              guide = ggplot2::guide_colorbar(barwidth = 0.8, barheight = 10, title.position = "top")) +
      ggspatial::annotation_scale(location = "bl", width_hint = 0.3, text_cex = 0.75) +
      ggspatial::annotation_north_arrow(location = "tr", which_north = "true",
                                        style = ggspatial::north_arrow_fancy_orienteering(fill = c("grey80", "grey20"), text_size = 8),
                                        height = ggplot2::unit(1.2, "cm"), width = ggplot2::unit(1.2, "cm")) +
      ggplot2::labs(title = title_str, subtitle = date_lbls[i], caption = "Nechako Watershed · Drought Analysis") +
      shared_ts_theme(11) +
      ggplot2::theme(
        plot.subtitle = ggplot2::element_text(size = 12, face = "bold", colour = "grey30"),
        legend.position = "right", panel.grid.major = ggplot2::element_line(colour = "grey92"),
        panel.grid.minor = ggplot2::element_blank(),
        plot.background = ggplot2::element_rect(fill = "white", colour = NA),
        panel.background = ggplot2::element_rect(fill = "#EEF4F8", colour = NA)
      ) +
    ...
    # 3. Render
    ragg::agg_png(filename = png_paths[i], width = FRAME_W, height = FRAME_H, res = FRAME_RES, units = "px")
    print(p)
    grDevices::dev.off()
