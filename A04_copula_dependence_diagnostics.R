...
#
#   FIX-A [this revision] plot_tv_parameter() referenced tv_result$yr_std,
#         but fit_timevarying_copula() never included yr_std/year_std in its
#         return list, so tv_result$yr_std was always NULL. This caused
#         seq(min(NULL * yr_sd + yr_mean), ...) to error whenever a
#         "tv" (time-varying) model was selected. year_std is now included
#         in every returned list (both the multi-parameter early-return path
#         and the main return path) as yr_std.
#
#   FIX-B [this revision] The multi-parameter (StudentT) early-return branch
#         of fit_timevarying_copula() hardcoded
#         cp_result = list(detected = FALSE, cp_year = NA) even though
#         detect_copula_changepoints() had already been run just above it in
#         the same function and its real result was sitting in cp_result.
#         This silently discarded genuine change-point detections (Pettitt /
#         CUSUM / PRUTF) for every StudentT-copula index/class, forcing
#         downstream non-stationary marginal fitting to always fall back to
#         the fixed 40%/30% epoch split instead of a detected break year.
#         The early-return now passes through the real cp_result.
#
#============================================================================
...
test_dependency <- function(drought_data, index_name) {
  ...
  log_event(sprintf("  [%s] Dependency — Kendall tau=%.4f (p=%.4e) | Spearman rho=%.4f",
...
#      yr_std           — standardised year vector matching uDat rows
#                          [FIX-A: added — plot_tv_parameter() needs this]
...
#                          [FIX-B: multi-parameter early-return now passes the
#                          real cp_result through instead of a hardcoded
...
fit_timevarying_copula <- function(drought_data, copula_fit_obj, marginal_fits,
  ...
  if (n_params > 1) {
    ...
    # FIX-B: pass through the real cp_result computed above (Pettitt/CUSUM/
    # PRUTF already ran) instead of hardcoding detected=FALSE, which was
    # silently discarding genuine change-point detections for every
    # multi-parameter (StudentT) copula index/class.
    # FIX-A: include yr_std so plot_tv_parameter() never sees a NULL field
    # (harmless here since selected_model is always "stationary" in this
    # branch, but keeps the return shape consistent with the main path).
  ...
  invisible(list(
    ...
    yr_mean      = yr_mean,
    yr_sd        = yr_sd,
    yr_std       = year_std,   # FIX-A: added — plot_tv_parameter() needs this
    link_ilink   = link_info$ilink,
...
plot_tv_parameter <- function(tv_result, index_name, output_dir) {
  ...
  # FIX-A: tv_result$yr_std is now populated by fit_timevarying_copula()
  # (previously always NULL, causing seq(min(NULL...), ...) to error here
  # whenever selected_model == "tv").
