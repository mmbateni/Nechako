...
(E) K_C Methodological Alignment [v6]:
derive_SAF_nonstationary_kendall() now mirrors the base pipeline's
approach: analytical Kendall distribution K_C(t) = t - φ(t)/φ'(t) is
used for Archimedean families (Frank, Gumbel, Joe, SurvClayton),
enabling valid extrapolation beyond the empirical support. Non-
Archimedean families (Gaussian, StudentT, Plackett) fall back to the
empirical estimator K_C(t) = mean(C(U,V) <= t). Optimization intervals
are adjusted accordingly (wider for analytical, tighter for empirical).
...
---------------------------------------------------------------------------
3. NON-STATIONARY SAF CURVES - KENDALL-CORRECTED  (v6 REVISED)
...
Changes vs v5:
* METHOD SPLIT: Aligns with base pipeline (Analysis.R). Analytical
K_C(t) = t - φ(t)/φ'(t) is applied for Archimedean families
(Frank, Gumbel, Joe, SurvClayton), enabling mathematically valid
extrapolation beyond the empirical support. Non-Archimedean
families (Gaussian, StudentT, Plackett) use the empirical estimator
K_C(t) = mean(C(U,V) <= t).
...
cop_obj_ns (the period-specific copula) when using the empirical path.
This is consistent with De Michele et al. (2013): K_C must match the
copula used in the SAF inversion, otherwise t_val is miscalibrated.
...
METHODOLOGICAL ALIGNMENT (v6):
Archimedean copulas possess closed-form generators φ(t) and derivatives φ'(t),
allowing exact evaluation of K_C(t) = t - φ(t)/φ'(t). This avoids the
discretization and support-limits inherent to empirical K_C, and permits
stable extrapolation for high return periods (T >= 50 yr).
Non-Archimedean families lack this property; we fall back to the empirical
estimator K_C(t) = P(C(U,V) <= t) evaluated on the fitted pseudo-observations.
...
if (use_analytical_KC) {
  # Extract dependence parameter theta from the period-specific copula object
  theta <- tryCatch(cop_obj_ns@parameters[1], error = function(e) NA_real_)
  if (is.na(theta) || !is.finite(theta)) {
    log_event(sprintf(
      "  [%s | %s] WARNING: Failed to extract valid theta for analytical K_C (family=%s). Falling back to empirical.",
...
if (!use_analytical_KC) {
  # Empirical K_C fallback for non-Archimedean families (Gaussian, StudentT, Plackett)
  ...
  cop_vals <- tryCatch(
    ...
    error = function(e) {
      log_event(sprintf(
        "  [%s | %s] pCopula failed for period-specific cop_obj_ns - falling back to full-record copula for empirical K_C.",
        index_name, period_label))
  ...
  log_event(sprintf(
    "  [%s | %s] Using empirical K_C for %s. Support limited to [1e-4, 1-1e-4].",
    index_name, period_label, cop_family))
}
T_years  <- c(10, 25, 50, 100)
area_pct <- seq(5, 95, by = 5)
