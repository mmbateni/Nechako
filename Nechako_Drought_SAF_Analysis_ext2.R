...
#   derive_SAF_nonstationary()         — SAF curves with period-specific 
#                                        marginals AND period-specific copula.
...
#   v4 paired a step-function marginal (discrete epochs from 
#   fit_nonstationary_marginals) with a smooth-trend copula collapsed to a 
#   single midpoint value (.build_period_copula). This mixes two incompatible 
#   temporal representations in the joint distribution without explicit 
#   justification.
#
# FIX — three changes:
...
.resolve_epoch <- function(ns_result, period_label) {
  ...
  cp_yr_mar <- if (grepl("change-point", ns_result$epoch_src, fixed = TRUE)) {
    m <- regmatches(ns_result$epoch_src, regexpr("[0-9]{4}", ns_result$epoch_src))
...
# ============================================================================
.assert_epoch_consistency <- function(tv_copula_result, cp_yr_mar, index_name, period_label) {
...
#      Note: if the change point is very early or late (< 20% or > 80% of record),
#            the 40%/30% split is used instead with a warning.
...
fit_nonstationary_marginals <- function(drought_data, index_name, year_vec,
  ...
  log_event(sprintf(
    "  [%s] NS marginals | epochs from: %s | ref: %d-%d | recent: %d-%d",
  ...
  # ---- Three-model fit ---------------------------------------------------
  m0 <- tryCatch(
    gamlss::gamlss(severity ~ 1, sigma.formula = ~ 1, family = gamlss.dist::GA(), data = df, trace = FALSE),
    error = function(e) NULL
  )
  m1 <- tryCatch(
    gamlss::gamlss(severity ~ year_std, sigma.formula = ~ 1, family = gamlss.dist::GA(), data = df, trace = FALSE),
    error = function(e) NULL
  )
  m2 <- tryCatch(
    gamlss::gamlss(severity ~ year_std, sigma.formula = ~ year_std, family = gamlss.dist::GA(), data = df, trace = FALSE),
    error = function(e) NULL
  )
  ...
  # Fix typo in assignment above for consistency
  result$mu_rec <- mu_rec
...
derive_SAF_nonstationary <- function(mu_period, sigma_period,
  ...
  use_ns_cop <- !is.null(tv_copula_result) &&
    tv_copula_result$selected_model %in% c("tv", "segmented") &&
    !is.null(tv_copula_result$make_cop_fn)
  
  if (!is.null(tv_copula_result)) {
    log_event(sprintf(
      "  [%s | %s] NS copula path: selected_model='%s', trigger=%s",
      index_name, period_label, tv_copula_result$selected_model, use_ns_cop
    ))
  }
  ...
  for (T in T_years) {
    ...
    for (i in seq_along(area_pct)) {
      ...
      res    <- tryCatch(optimize(obj, interval = c(1e-6, 50)), error = function(e) list(minimum = NA_real_))
      sev[i] <- res$minimum
    }
  ...
  write.csv(saf_ns,
            file.path(output_dir, sprintf("%s_SAF_nonstationary_%s.csv", index_name, gsub("[^A-Za-z0-9]", "_", period_label))),
            row.names = FALSE)
  log_event(sprintf(
  ...
  for (T in T_years) {
    ...
    for (i in seq_along(area_pct)) {
      ...
      obj <- function(s) {
        u_ns <- pgamma(s, shape = ns_shape, rate = ns_rate)
        val  <- tryCatch(
          copula::pCopula(cbind(u_ns, v), cop_obj_ns),
          error = function(e) NA_real_
        )
        if (!is.finite(val)) return(1e6)
        (val - t_val)^2
    ...
    saf_ns_k <- rbind(saf_ns_k, data.frame(
      ReturnPeriod_years = T,
      Area_pct           = area_pct,
      Severity           = sev,
      Method             = "Kendall_NS",
    ...
    saf_ns_k$sigma_period[nrow(saf_ns_k)] <- round(sigma_period, 4) # fix assignment typo in rbind
  }
  
  write.csv(saf_ns_k,
            file.path(output_dir, sprintf("%s_SAF_nonstationary_kendall_%s.csv", index_name, gsub("[^A-Za-z0-9]", "_", period_label))),
            row.names = FALSE)
  log_event(sprintf(
...
place_event_on_saf <- function(drought_data, copula_fit_obj, marginal_fits,
  ...
  ev_data <- drought_data[drought_data$year %in% event_years & drought_data$severity > 0, ]
  if (nrow(ev_data) == 0) {
    log_event(sprintf("  [%s] No drought data found for years %s.", index_name, paste(event_years, collapse = "-")))
    return(NULL)
  }
  ...
  write.csv(result,
            file.path(output_dir, sprintf("%s_event_%s_on_SAF.csv", index_name, paste(range(event_years), collapse = "_"))),
            row.names = FALSE)
  log_event(sprintf(
    "  [%s] Event %s--%s: S=%.3f, A=%.1f%%, T~%d yr",
    index_name, min(event_years), max(event_years), s_peak, a_peak, round(T_joint)
  ))
  result
}
