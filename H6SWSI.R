  ...
  if (nrow(reservoir) > 12) {
    p <- ggplot(reservoir, aes(x = date, y = storage)) +
      ...
      labs(title = "Nechako Basin — GloLakes Reservoir Storage",
           subtitle = sprintf("Best-estimate from %.1f lakes (mean) | Q1+Q2 (absolute) only", 
...
load_era5_basin_mean <- function(nc_path, var_name,
  ...
  out <- tibble::tibble(
    ...
    water_year = dplyr::if_else(lubridate::month(dates) >= WATER_YEAR_START_MONTH,
...
build_forecast_models <- function(era5_data, monthly_vol_primary) {
  ...
  cum_melt <- monthly_vol_primary %>%
    ...
    ungroup() %>%
    dplyr::select(water_year, month, cum_obs_to_month)
  
  prev_q <- monthly_vol_primary %>%
    arrange(year, month) %>%
    mutate(prev_volume_Mm3 = lag(volume_Mm3, 1)) %>%
    dplyr::select(year, month, prev_volume_Mm3)
  
  master <- era5_data %>%
    dplyr::select(year, month, water_year, swe_corrected_Mm3, precip_ytd_Mm3) %>%
  ...
  for (m in 1:12) {
    ...
    # Skip if target has zero (or near-zero) variance — regression and Weibull
    # CDF both break when all target values are identical (e.g. September
    # remaining-melt volume is always 0 because Sep is the last melt month).
    if (var(train$target_Mm3, na.rm = TRUE) < 1e-6) {
      cat(sprintf("skipped — target has zero variance (all values = %.4f)\n",
                  mean(train$target_Mm3, na.rm = TRUE)))
      models[[m]] <- NULL
      next
    }
    ...
    all_preds <- master %>%
      ...
      dplyr::select(year, month, water_year, forecast_Mm3, loocv_r2, loocv_rmse, forecast_type)
...
compute_weibull_prob <- function(hist_vals, x_new) {
  ...
  # approx() requires at least 2 distinct x values; if all historical values
  # are identical (e.g. Sep remaining-melt volume is always 0), return the
  # 50th-percentile neutral value rather than crashing.
  if (length(unique(v)) < 2) return(50)
  P <- seq_len(n) / (n + 1) * 100
...
fit_parametric_cdf <- function(hist_vals) {
  ...
  # L-moments and distribution fitting require variance; bail out gracefully.
  if (var(v) < 1e-6) return(NULL)
...
compute_swsi <- function(era5_data, monthly_vol, forecast_result, 
  ...
  combined <- monthly_vol %>%
    ...
    left_join(forecast_result$forecasts %>% 
                dplyr::select(year, month, forecast_Mm3, forecast_type),
              by = c("year","month")) %>%
    left_join(era5_data %>% dplyr::select(year, month, swe_corrected_Mm3, 
  ...
  if (RESERVOIR_DATA_AVAILABLE && !is.null(reservoir_data)) {
    ...
    combined <- combined %>%
      left_join(reservoir_data %>% dplyr::select(year, month, storage),
                by = c("year","month"))
...
identify_swsi_droughts <- function(swsi_df, station_id=NULL) {
  ...
  # Safe min: returns NA (not Inf) when all inputs are NA, suppressing warnings
  # from min(na.rm=TRUE) on all-NA vectors (e.g. idx_forecast when Sep has no model).
  safe_min <- function(x) {
    x <- x[!is.na(x)]
  ...
  event_info <- data %>%
    ...
    summarise(
      ...
      swsi_min         = safe_min(swsi),
      swsi_mean        = mean(swsi, na.rm=TRUE),
      idx_swe_min      = safe_min(idx_swe),
      idx_precip_min   = safe_min(idx_precip),
      idx_forecast_min = safe_min(idx_forecast),
      .groups = "drop"
    ) %>%
    ...
    mutate(
      ...
      primary_driver = case_when(
        !is.na(idx_swe_min) & !is.na(idx_forecast_min) &
          idx_swe_min <= idx_forecast_min & idx_swe_min <= replace_na(idx_precip_min, Inf) ~ "SWE deficit",
        !is.na(idx_precip_min) & (is.na(idx_forecast_min) | idx_precip_min <= idx_forecast_min) ~ "Precipitation deficit",
        !is.na(idx_forecast_min) ~ "Streamflow forecast deficit",
        TRUE ~ "Unknown (no forecast data)"
      )
    )
...
cat(sprintf("Reservoir:    GloLakes v2 (mean %.0f Mm³, %.0f lakes)\n",
