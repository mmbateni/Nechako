####################################################################################
# 4pr_pet_trends_v2.r  ??  Trend Analysis ??? Precipitation, PET (FAO-56 PM + ERA5-Land PEV), Temperature
# ???????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????
...
#   ??? PET_ERA5         (ERA5-Land PEV, mm/month ??? warm-season sum Apr???Oct)
#   ??? Temperature     (ERA5-Land 2 m Tair, K?????C, annual mean + 12 calendar months)
#
# MODIFICATIONS vs. original:
#   [NEW] ERA5-Land PEV PET (PET_ERA5) is processed in parallel with PM PET.
...
#   all_results.rds              pixel + basin trend statistics (Pr, PET, PET_ERA5, Tair)
#   analysis_metadata.rds        time series, extents, parameters (incl. PET_ERA5)
...
#   point_monthly_timeseries.csv raw monthly Pr / PET / PET_ERA5 / Tair at each point
#   data_processing.log
####################################################################################
...
# [NEW] ERA5-Land PEV PET ??? produced by 2preq_PET_ERALand.R (Section 5b)
pet_era5_path  <- "monthly_data_direct/potential_evaporation_era5land_monthly.nc"
...
####################################################################################
log_event("Loading Precipitation, PET (PM and ERA5-Land PEV), and Temperature data...")

precip_full <- rast(precip_path)
pet_full    <- rast(pet_path)

# [NEW] ERA5-Land PEV PET
if (!file.exists(pet_era5_path))
  stop("ERA5-Land PEV PET NetCDF not found: ", pet_era5_path,
       "\nRun 2b_PET_ERALand_v2.R first.")
pet_era5_full <- rast(pet_era5_path)
log_event(sprintf("  [PET_ERA5] Loaded: %d layers from %s", nlyr(pet_era5_full), basename(pet_era5_path)))
...
log_event("Converting Pr, PET (PM), and PET (ERA5) from mm/day to mm/month...")
...
for (i in seq_len(nlyr(pet_era5_full)))  pet_era5_full[[i]]  <- pet_era5_full[[i]]  * days_in_month[i]
...
pet_era5_full  <- project(pet_era5_full,  "EPSG:3005", method="bilinear")  # [NEW]
...
pet_era5_full <- resample(pet_era5_full, analysis_template, method="bilinear")
...
pet_era5_clipped <- mask(pet_era5_full, mask_r, maskvalues = 0, updatevalue = NA)
...
pet_era5_monthly_avg   <- as.vector(global(pet_era5_clipped, fun="mean", na.rm=TRUE)[,1])  # [NEW]
...
pet_era5_annual_avg_matrix  <- aggregate_to_annual_warmseason_fast(          # [NEW]
  matrix(pet_era5_monthly_avg, ncol=1), years, months_vec=months,
  warm_months=warm_months_pet, method="sum")
...
pet_era5_annual_avg   <- pet_era5_annual_avg_matrix[,1]  # [NEW]
...
####################################################################################
# ERA5-Land PEV does not use the Thornthwaite 0°C floor. No floor-crossing
# diagnostic is applied here.

# Pre-process PET (both): replace zeros with min_positive_value
for (clip_r in list(pet_clipped, pet_era5_clipped)) {
  pv  <- values(clip_r)
...
pet_era5_matrix     <- t(values(pet_era5_clipped)[valid_mask,   , drop=FALSE])  # [NEW]
...
log_event(sprintf("  Matrices: Pr/PET %d??%d | PET_ERA5 %d??%d | Tair %d??%d",
                  nrow(precip_matrix), ncol(precip_matrix),
                  nrow(pet_era5_matrix), ncol(pet_era5_matrix),
...
log_event("Processing basin-averaged series (Pr, PET_PM, PET_ERA5, Temperature)...")
...
log_series_quality(pet_era5_monthly_avg, "Basin PET_ERA5 monthly", min_positive_value)
...
log_series_quality(pet_era5_annual_avg,  "Basin PET_ERA5 annual", min_positive_value)
...
basin_pet_results       <- process_basin_series(pet_monthly_avg,     pet_annual_avg,
                                                "PET", is_precip=FALSE)
basin_pet_era5_results   <- process_basin_series(pet_era5_monthly_avg, pet_era5_annual_avg,  # [NEW]
                                                "PET_ERA5", is_precip=FALSE)
...
pet_results      <- process_variable_final(pet_matrix,     "PET",       coords_dt,
                                           is_precip=FALSE)
log_event("=== STARTING PET (ERA5-Land PEV) TREND ANALYSIS ===")   # observed framework
pet_era5_results  <- process_variable_final(pet_era5_matrix, "PET_ERA5",   coords_dt,
                                           is_precip=FALSE)
...
####################################################################################
log_event("=== SPECIFIC POINT ANALYSIS (Pr, PET_PM, PET_ERA5, Temperature) ===")
...
pt_pet_era5 <- extract_point_monthly(pet_era5_clipped, pts_proj, dates,   "PET_ERA5",       "mm/month")  # [NEW]
...
point_monthly_ts <- rbindlist(list(pt_pr, pt_pet, pt_pet_era5, pt_tair),
...
pt_pet_era5_trends <- run_point_trends(pt_pet_era5, "PET_ERA5",       is_precip=FALSE,   # [NEW]
                                      warm_season_annual=TRUE)
pt_tair_trends    <- run_point_trends(pt_tair,    "Temperature",   is_precip=FALSE,
                                      skip_min_filter=TRUE)

point_trend_stats <- rbindlist(
  list(pt_pr_trends, pt_pet_trends, pt_pet_era5_trends, pt_tair_trends),
  use.names=TRUE, fill=TRUE)
...
all_results <- rbindlist(
  list(precip_results,     pet_results,     pet_era5_results,  tair_results,
       basin_precip_results, basin_pet_results, basin_pet_era5_results, basin_tair_results),
  use.names=TRUE, fill=TRUE)
...
saveRDS(list(
  ...
  basin_avg_monthly = data.frame(
    ...
    pet_era5_mm_month   = pet_era5_monthly_avg,   # [NEW]
  ...
  basin_avg_annual = data.frame(
    ...
    pet_era5_mm_year    = pet_era5_annual_avg,     # [NEW]
...
log_event("????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????")
log_event("DATA PROCESSING COMPLETE (Pr + PET_PM + PET_ERA5 + Temperature)")
cat("\n??? DATA PROCESSING COMPLETED SUCCESSFULLY\n")
cat(sprintf("  Basin pixels: %d  |  Specific points: %d\n", n_basin_pixels, n_pts))
cat(sprintf("  Variables: Precipitation, PET (PM), PET (ERA5-Land PEV), Temperature\n"))
...
cat(sprintf("  point_monthly_timeseries.csv ??? raw monthly Pr / PET_PM / PET_ERA5 / Tair\n"))
