####################################################################################
# 5gif_animations.R · ANIMATED GIFS — OBSERVED DROUGHT FRAMEWORK (Optimised & Enhanced)
# Framework: SPI + SPEI-PM + SPEI-ERA5L + SWEI. No detrended/counterfactual branch.
####################################################################################
...
render_gif <- function(index_type, scale, data_dir, find_fn, basin_shp,
  ...
  label <- if (index_type == "spei") "SPEI_PM" else
           if (index_type == "spei_era5") "SPEI_ERA5L" else toupper(index_type)
  tag <- if (is.na(scale)) label else sprintf("%s-%d", label, scale)
  title_str <- if (index_type == "spei") {
    sprintf("SPEI-PM-%d (Penman-Monteith)", scale)
  } else if (index_type == "spei_era5") {
    sprintf("SPEI-ERA5L-%d (ERA5-Land Potential Evaporation)", scale)
  } else tag
...
# Observed-framework jobs only. SPEI-PM and SPEI-ERA5L are parallel observed
# formulations and are rendered with identical settings for direct comparison.
SPEI_ERA5L_SEAS_DIR <- file.path(WD_PATH, "spei_results_seasonal_era5land")
SPEI_ERA5L_SCALES   <- SPEI_SCALES

find_spei_era5l_nc_safe <- function(data_dir, index_type, scale) {
  # Preferred naming written by the revised SPEI workflow.
  out <- tryCatch(
    find_seasonal_nc_files(data_dir, "spei_era5", scale),
    error = function(e) character(0)
  )

  # Accept explicit ERA5-Land naming if the SPEI workflow writes files
  # with the spei_era5land prefix.
  if (!length(out)) {
    out <- tryCatch(
      find_seasonal_nc_files(data_dir, "spei_era5land", scale),
      error = function(e) character(0)
    )
  }

  # Final fallback for a dedicated ERA5-Land directory where files retain
  # the generic SPEI prefix. Because the directory is separate from
  # spei_results_seasonal/, this cannot mix with SPEI-PM files.
  if (!length(out)) {
    out <- tryCatch(
...
jobs <- c(
  lapply(SPI_SCALES, function(sc)
    list("spi", sc, SPI_SEAS_DIR, find_seasonal_nc_files)),
  lapply(SPEI_SCALES, function(sc)
    list("spei", sc, SPEI_SEAS_DIR, find_seasonal_nc_files)),
  if (dir.exists(SPEI_ERA5L_SEAS_DIR))
    lapply(SPEI_ERA5L_SCALES, function(sc)
      list("spei_era5", sc, SPEI_ERA5L_SEAS_DIR, find_spei_era5l_nc_safe))
  else list(),
