...
# Paths
data_folder <- "./Hydrology/data_retrievalWaterSurveyofCanada/data_downloads_geomet_api" 
output_folder <- "./Hydrology/primary_plots/" 
stations_metadata_file <- "./Hydrology/data_retrievalWaterSurveyofCanada/stations.csv" # The file you uploaded

if(!dir.exists(output_folder)) dir.create(output_folder, recursive = TRUE)

# 2. Load Station Metadata for naming
# We create a lookup table: Station Number -> Station Name
stn_metadata <- read_csv(stations_metadata_file, show_col_types = FALSE) %>%
  select(STATION_NUMBER, STATION_NAME) %>%
  distinct()

# 3. Get list of all CSV files
files <- list.files(path = data_folder, pattern = "\\.csv$", full.names = TRUE)

# --- Helper Function: Detect Gaps ---
get_data_gaps <- function(df) {
  ...
  is_missing <- is.na(df_full$Value)
...
process_and_plot_SWE <- function(file_path) {
  ...
  df_clean <- raw_data %>%
    mutate(
      Date = parse_date_time(Date, orders = c("ymd", "mdy", "dmy", "Ymd HMS")),
      Grade = str_to_title(trimws(Grade))  # Normalize grade labels
    ) %>%
    filter(!is.na(Date)) %>%
    arrange(Date)
...
walk(files, function(f) {
  tryCatch({
    process_and_plot_named(f, stn_metadata)
  }, error = function(e) {
