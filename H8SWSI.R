...
load_reservoir_storage <- function() {
  ...
  reservoir <- raw %>%
    dplyr::mutate(
      ...
      year       = year(date),
      month      = month(date),
      water_year = dplyr::if_else(month >= WATER_YEAR_START_MONTH, year, year - 1L),
      n_lakes    = as.numeric(n_lakes_contributing),
      products   = product                # ← fixed: column is "product" not "products_used"
    ) %>%
    dplyr::filter(!is.na(storage), storage >= 0, n_lakes >= 1) %>%
  ...
  if (nrow(reservoir) > 12) {
    p <- ggplot(reservoir, aes(x = date, y = storage)) +
      ...
      labs(title = "Nechako Basin — GloLakes Reservoir Storage",
           subtitle = sprintf("Best-estimate from %.1f lakes | Q1+Q2 (absolute) only", 
...
load_naturalized_flow <- function(station_id, nat_path = INPUT_NATURAL_DIR) {
  ...
  data <- raw %>%
    ...
    dplyr::filter(!is.na(date), tolower(symbol) == "s", !is.na(discharge), discharge >= 0) %>%
