...
derive_SAF_nonstationary <- function(mu_period, sigma_period,
  ...
  if (is.null(cop_obj_ns)) {
    cop_obj_ns  <- .fix_tcopula_df(copula_fit_obj$best_copula_fit@copula)  # FIX: round tCopula df
    copula_path <- "stationary"
  }
...
place_event_on_saf <- function(drought_data, copula_fit_obj, marginal_fits,
  ...
  cop_obj  <- .fix_tcopula_df(copula_fit_obj$best_copula_fit@copula)  # FIX: round tCopula df
  beta_par <- marginal_fits$area_fit$estimate
...
# ============================================================================
# 5. PLACE A HISTORICAL EVENT ON THE SAF SURFACE — NON-STATIONARY  [NEW v8]
# Extends place_event_on_saf() (stationary, peak-month only) with two upgrades
# motivated by MS4 Section 2.9 and the companion manuscripts' univariate
# effective-return-period finding:
#
#   (a) DUAL CHARACTERISATION — computes the joint return period using both
#       the event's peak-month (S,A) [as in place_event_on_saf()] AND its
#       event-mean (S,A) averaged over the full below-threshold period.
#       The two characterisations bound the joint RP: peak-month is the
#       more extreme (shorter RP), event-mean is more representative of the
#       episode as a whole.
#
#   (b) STATIONARY *AND* NON-STATIONARY JOINT RP — in addition to the
#       stationary joint RP (survival-copula formula, matching MS4 eq. in
#       Sec. 2.9), this function also evaluates the joint RP under the
#       recent-period non-stationary marginal (from fit_nonstationary_
#       marginals()) and the recent-period copula (from
#       .build_period_copula(), reusing the same three-path priority logic
#       already used by derive_SAF_nonstationary()). This is the bivariate
#       analogue of the univariate "effective return period compression"
#       finding in the companion manuscripts — nothing in the existing
#       pipeline currently produces it.
#
# Falls back gracefully to stationary-only output if ns_result /
# tv_copula_result are not supplied (i.e. behaves like a superset of the
# original place_event_on_saf(), which is left untouched above for
# backward compatibility with any code that still calls it directly).
#
# Parameters:
# drought_data      : full drought-characteristics data frame (output of
#                      extract_drought_characteristics_spi)
# copula_fit_obj    : stationary copula result from fit_copulas()
# marginal_fits     : stationary marginal result from fit_marginal_distributions()
# event_start, event_end : Date (or coercible) bounds of the event window
# index_name        : character label for logging / file naming
# output_dir        : directory for CSV output
# ns_result         : optional; output of fit_nonstationary_marginals().
#                      Required for the non-stationary comparison.
# tv_copula_result  : optional; output of fit_timevarying_copula().
#                      Required for the non-stationary comparison.
# period_label      : which ns_result epoch to compare against; must match
#                      the label passed to .resolve_epoch() (default "recent")
# ============================================================================
place_event_on_saf_v2 <- function(drought_data, copula_fit_obj, marginal_fits,
                                  event_start, event_end,
                                  index_name, output_dir,
                                  ns_result = NULL, tv_copula_result = NULL,
                                  period_label = "recent") {
  if (is.null(copula_fit_obj) || is.null(marginal_fits)) return(NULL)
  
  event_start <- as.Date(event_start); event_end <- as.Date(event_end)
  ev_data <- drought_data[drought_data$date >= event_start &
                            drought_data$date <= event_end &
                            drought_data$severity > 0, ]
  if (nrow(ev_data) == 0) {
    log_event(sprintf("  [%s] No drought-active months found in window %s to %s.",
                      index_name, event_start, event_end))
    return(NULL)
  }
  
  # ---- Two event characterisations -----------------------------------------
  peak_i    <- which.max(ev_data$severity)
  char_list <- list(
    peak_month = list(severity = ev_data$severity[peak_i],
                      area_pct = ev_data$area_pct[peak_i]),
    event_mean = list(severity = mean(ev_data$severity, na.rm = TRUE),
                      area_pct = mean(ev_data$area_pct, na.rm = TRUE))
  )
  
  # ---- mu_T: renewal-adjusted mean waiting time (months), matching the
  #      convention used elsewhere in the pipeline (SAF_Analysis.R Sec 3f,
  #      derive_SAF_nonstationary()) rather than the plain Poisson n_tot/n_dr
  #      used by the original place_event_on_saf(). ---------------------------
  active    <- drought_data$severity > 0 & drought_data$area_pct > 0
  n_tot     <- nrow(drought_data); n_dr <- sum(active)
  mu_T_adj  <- (n_tot / n_dr) * 12
  is_start  <- active & !c(FALSE, head(active, -1))
  ev_starts <- as.Date(drought_data$date[is_start])
  if (length(ev_starts) >= 2) {
    iat_yrs <- as.numeric(diff(ev_starts)) / 365.25
    if (mean(iat_yrs, na.rm = TRUE) > 0) mu_T_adj <- mean(iat_yrs, na.rm = TRUE) * 12
  }
  
  beta_par <- marginal_fits$area_fit$estimate
  cop_stat <- .fix_tcopula_df(copula_fit_obj$best_copula_fit@copula)
  
  joint_RP <- function(s, a, cop_obj, F_S) {
    u_s     <- F_S(s)
    u_a     <- pbeta(a / 100, beta_par[1], beta_par[2])
    C_val   <- tryCatch(copula::pCopula(cbind(u_s, u_a), cop_obj), error = function(e) NA_real_)
    p_joint <- max(0, 1 - u_s - u_a + C_val)
    T_joint <- if (p_joint > 0) mu_T_adj / (p_joint * 12) else Inf
    c(u_s = u_s, u_a = u_a, C = C_val, p_joint = p_joint, T_years = T_joint)
  }
  F_S_stationary <- function(s) compute_u_severity(s, marginal_fits)
  
  # ---- Non-stationary setup (optional) --------------------------------------
  have_ns <- !is.null(ns_result) && !is.null(tv_copula_result)
  cop_ns  <- NULL; F_S_ns <- NULL; copula_path <- "not_evaluated"
  if (have_ns) {
    epoch <- .resolve_epoch(ns_result, period_label)
    .assert_epoch_consistency(tv_copula_result, epoch$cp_yr_mar, index_name, period_label)
    mu_rec    <- if (epoch$which_seg == "ref") ns_result$mu_ref    else ns_result$mu_rec
    sigma_rec <- if (epoch$which_seg == "ref") ns_result$sigma_ref else ns_result$sigma_rec
    if (!is.null(mu_rec) && !is.null(sigma_rec) && is.finite(mu_rec) && is.finite(sigma_rec)) {
      ns_shape <- 1 / (sigma_rec^2); ns_rate <- ns_shape / mu_rec
      F_S_ns   <- function(s) pgamma(s, shape = ns_shape, rate = ns_rate)
      use_tv   <- isTRUE(tv_copula_result$significant) && !is.null(tv_copula_result$make_cop_fn)
      if (use_tv) {
        cop_ns <- tryCatch(.build_period_copula(tv_copula_result, epoch$period_yrs, epoch$which_seg),
                           error = function(e) NULL)
      }
      copula_path <- if (!is.null(cop_ns)) {
        if (!is.null(tv_copula_result$seg_result)) "segmented" else "tv_midpoint"
      } else "stationary"
      if (is.null(cop_ns)) cop_ns <- cop_stat
    } else {
      log_event(sprintf("  [%s] Non-stationary marginal parameters unavailable for period '%s' — non-stationary comparison skipped.",
                        index_name, period_label))
      have_ns <- FALSE
    }
  }
  ...
  # ---- Assemble results -------------------------------------------------------
  out <- data.frame()
  for (cname in names(char_list)) {
    ch     <- char_list[[cname]]
    r_stat <- joint_RP(ch$severity, ch$area_pct, cop_stat, F_S_stationary)
    out <- rbind(out, data.frame(
      index            = index_name,
      event_window     = sprintf("%s_to_%s", event_start, event_end),
      characterisation = cname,
      severity         = round(ch$severity, 4),
      area_pct         = round(ch$area_pct, 2),
      model            = "stationary",
      period_label     = "full_record",
      copula_path      = "stationary",
      p_joint          = round(unname(r_stat["p_joint"]), 6),
      T_joint_years    = round(unname(r_stat["T_years"]), 1)))
    
    if (have_ns) {
      r_ns <- joint_RP(ch$severity, ch$area_pct, cop_ns, F_S_ns)
      out  <- rbind(out, data.frame(
        index            = index_name,
        event_window     = sprintf("%s_to_%s", event_start, event_end),
        characterisation = cname,
        ...
        copula_path      = copula_path,
        p_joint          = round(unname(r_ns["p_joint"]), 6),
        T_joint_years    = round(unname(r_ns["T_years"]), 1)))
    }
  }
  
  write.csv(out,
            file.path(output_dir, sprintf("%s_event_%s_%s_on_SAF_v2.csv",
                                          index_name,
                                          format(event_start, "%Y"),
                                          format(event_end,   "%Y"))),
            row.names = FALSE)
  
  for (i in seq_len(nrow(out))) {
    log_event(sprintf(
      "  [%s] %s | %s (%s, %s): T_joint = %s yr",
      index_name, out$characterisation[i], out$model[i], out$period_label[i],
      out$copula_path[i],
      if (is.finite(out$T_joint_years[i])) round(out$T_joint_years[i]) else "Inf"))
  }
  out
}

# Log successful load of extension script v8
log_event("Extensions 2 (v8) loaded: fit_nonstationary_marginals | derive_SAF_nonstationary | derive_SAF_nonstationary_kendall | place_event_on_saf | place_event_on_saf_v2")